...
const App = () => {
    ...
    // 📍 แก้บัคหน้าจอค้าง: เพิ่ม State ที่ขาดหายไปเพื่อรองรับ Modal Admin Cell
    ...
    // 📍 ฟังก์ชันบันทึกประวัติการทำงานลง Firebase 100% (แทนที่ Google Sheets log)
    ...
    useEffect(() => {
        if (successModal) {
            // 📍 เพิ่มเวลาให้ Pop-up โชว์นานขึ้นเป็น 3.5 วินาที จะได้อ่านรายละเอียดทัน
            const timer = setTimeout(() => { setSuccessModal(null); }, 3500);
            return () => clearTimeout(timer);
        }
    }, [successModal]);

    // จัดการค่าขนาด UI บน LocalStorage
    ...
    // 📍 1. แก้ไขลอจิกระบบแจ้งเตือน: กรองข้อมูลเฉพาะที่ยังไม่ได้อ่าน
    ...
    useEffect(() => {
        ...
        // 3. ดึงข้อมูล User/Logs (เฉพาะแอดมิน)
        ...
        return () => {
            unsubBookings(); unsubInspectors(); unsubSettings();
            if (unsubUsers) unsubUsers(); if (unsubLogs) unsubLogs();
        };
    ...
    const handleTouchEnd = async () => {
        if (pullY > 50 && !isRefreshing) { 
            ...
            setPullY(50); 
            // จำลองดีเลย์นิดหน่อยเพื่อบอกว่าอัปเดตแล้ว เพราะ Firebase มันอัปเดตตลอดเวลาอยู่แล้ว
    ...
    // ค้นหา ID อัตโนมัติถ้าไม่มี ID (ป้องกัน Error ไม่พบไอดีใน Firebase)
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    // 📍 แสดงรายละเอียดรายการที่ถูกลบใน Pop-up แจ้งเตือน
                    setSuccessModal(
    ...
    // 📍 หน้าจอ Login (ระบบ Firebase 100%)
    if (!user) {
        return (
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                setLoadingMsg('กำลังค้นหาบัญชีบน Firebase...');
                                try {
                                    if (window.dbFirestore) {
    ...
    if (!isFirebaseReady) return <div className="h-screen w-full flex items-center justify-center flex-col gap-4 p-8 text-center"><Icons.Loader /><h2 className="text-xl font-bold text-slate-800">กำลังเชื่อมต่อ Firebase...</h2></div>;
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 Overlay แจ้งเตือนความสำเร็จ (พร้อมแสดงรายละเอียดครบถ้วน) */}
            {successModal && (
                    ...
                    <div className="grid-wrapper" ref={scrollRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ transform: `translateY(${pullY}px)`, transition: pullY === 0 ? 'transform 0.3s ease-out' : 'none' }}>
                        {initialLoad ? <div className="w-full h-full p-4"><div className="w-full h-16 skeleton rounded-lg bg-slate-100 animate-pulse"></div></div> : (
                            <CalendarGrid 
                                ...
                                setQuickAddType={setQuickAddType} filteredBookings={filteredBookings}
                ...
                <div className="page-view relative pb-20 animate-pop">
                    ...
                    {initialLoad ? (
                        ...
                        <div className="space-y-4 pb-10">
                            ...
                            {(() => {
                                // Data Processing (อิงจาก db.bookings แบบ Realtime 100%)
                ...
                <div className="page-view relative pb-20">
                    ...
                    {showAdminHelp && (
                        <div className="bg-blue-50 p-5 rounded-2xl border border-blue-200 mb-4 shadow-sm text-sm animate-pop">
                            ...
                            <div className="space-y-4 text-blue-900 text-xs leading-relaxed">
                                ...
                                <div><b>🗓️ จัดการวันพิเศษ (ลาก/ย้ายคิว):</b> แอดมินสามารถกำหนดวันหยุดบริษัท, กิจกรรม (แสดงแถบสีเขียว), และวันลา (แสดงแถบสีเหลือง) <br/> แอดมินสามารถ <b>คลิกค้างที่การ์ดแล้วลาก (Drag & Drop)</b> เพื่อย้ายคิวงานข้ามวันหรือเปลี่ยนคนตรวจได้ทันที</div>
                        ...
                        <div className="space-y-4 animate-pop pb-10">
                                ...
                                <button onClick={() => {
                                    setConfirmDialog({
                                        ...
                                        onConfirm: async () => {
                                            ...
                                            setSuccessModal(
                                                <div className="text-center">
                                                    <div className="font-black text-sm mb-1 text-red-600">รีเซ็ตการตั้งค่าแล้ว</div>
                                                    <div className="text-xs text-slate-600">ระบบกลับไปใช้สีและขนาดเริ่มต้น</div>
                                                </div>
                                            );
                                        }
                                    });
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    setSuccessModal(
                                        <div className="text-center">
                                            <div className="font-black text-sm mb-1 text-green-600">บันทึกสำเร็จ!</div>
                                            <div className="text-xs text-slate-600">อัปเดตสไตล์เว็บไซต์เรียบร้อยแล้ว</div>
                                        </div>
                                    );
                                } catch(err) {
                    ...
                    {adminTab === 'users' && (
                        <div className="space-y-3 mt-4 animate-pop pb-10">
                            ...
                            {adminDb.users.map((u, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2">
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold text-slate-800 text-sm">{u.username} <span className="text-[9px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{u.role}</span></div>
                                                <div className="text-[10px] text-slate-500">{u.full_name || '-'}</div>
                                            </div>
                                        </div>
                                        <div className="text-[10px] font-bold px-3 py-1.5 rounded-lg border bg-slate-50">
                                            {u.status === 'approved' ? <span className="text-emerald-600">✅ อนุมัติแล้ว</span> : <span className="text-amber-600">⏳ รออนุมัติ</span>}
                                        </div>
                                    </div>
                                    
                                    <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        <button onClick={() => setModal({ type: 'edit_user', isNew: false, data: u })} className="text-[11px] bg-blue-50 text-blue-600 py-2 rounded-lg font-bold border border-blue-200 col-span-2">แก้ไข/เปลี่ยนสิทธิ์</button>
                                        <button onClick={async () => {
                                            const newStatus = u.status === 'approved' ? 'pending' : 'approved';
                                            if (window.dbFirestore) await window.dbFirestore.collection("users").doc(u.username).update({ status: newStatus });
                                            setSuccessModal('อัปเดตสถานะสำเร็จ');
                                        }} className="text-[11px] bg-amber-50 text-amber-600 py-2 rounded-lg font-bold border col-span-1">เปลี่ยนสถานะ</button>
                                        <button onClick={() => {
                                            setConfirmDialog({
                                        ...
                                        }} className="text-[11px] bg-red-50 text-red-600 py-2 rounded-lg font-bold border col-span-1">ลบ</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {adminTab === 'inspectors' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <div className="bg-gradient-to-br from-indigo-800 to-indigo-600 p-5 rounded-2xl shadow-md text-white mb-6">
                                ...
                                <button onClick={() => setModal({ type: 'inspector_form' })} className="w-full py-3 bg-white text-indigo-700 font-black rounded-xl shadow-lg flex items-center justify-center gap-2">
                                    <Icons.UserPlus /> เพิ่มรายชื่อผู้ตรวจใหม่
                                </button>
                            </div>
                            <div className="space-y-3">
                                {(db.inspectors || []).map((ins, i) => (
                                    <div key={i} className="bg-white p-4 rounded-2xl shadow-sm border flex justify-between items-center">
                                        <div className="font-black text-slate-800 text-base">{ins.name}</div>
                                        <div className="flex gap-2">
                                            <button onClick={() => setModal({ type: 'inspector_form', data: ins })} className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Icons.Edit /></button>
                                            <button onClick={() => setConfirmDialog({msg:`ยืนยันการลบผู้ตรวจ "${ins.name}"?`, onConfirm: async() => {
                                                setConfirmDialog(null);
                    ...
                    {adminTab === 'special_management' && (
                        <div className="animate-pop pb-10 space-y-4">
                            <button onClick={() => setModal({ type: 'manage_leaves' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center"><Icons.User /></div>
                                <span className="font-bold text-amber-800 text-sm text-left flex-1">จัดการวันลา</span>
                            </button>
                            <button onClick={() => setModal({ type: 'manage_events' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center"><Icons.Star /></div>
                                <span className="font-bold text-emerald-800 text-sm text-left flex-1">จัดการกิจกรรมบริษัท</span>
                            </button>
                            <button onClick={() => setModal({ type: 'manage_holidays' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-red-50 text-red-600 flex items-center justify-center"><Icons.CalendarX /></div>
                                <span className="font-bold text-red-800 text-sm text-left flex-1">จัดการวันหยุดบริษัท</span>
                            </button>
                        </div>
                    )}
                    ...
                    {adminTab === 'all_bookings' && (
                        <div className="space-y-3 mt-4 animate-pop">
                            {(adminDb.all_bookings || []).slice(0, adminBookingsLimit).map((h, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border relative">
                                    <div className="font-bold text-slate-800 text-sm mb-1">{h.site_name || '-'} ({h.date ? formatSafeDate(h.date) : '-'})</div>
                                    <div className="text-xs text-slate-500">Eq No: {h.equipment_no} | ผู้ตรวจ: {h.inspector_name}</div>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                    ...
                    {modal?.type === 'edit_user' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <h3 className="text-xl font-bold text-slate-900 mb-4 border-b pb-2">{modal.isNew ? 'สร้างผู้ใช้งานใหม่' : 'แก้ไขข้อมูลผู้ใช้งาน'}</h3>
                            <form onSubmit={async (e) => {
                                e.preventDefault();
                                const fd = new FormData(e.target);
                                const usernameInput = modal.isNew ? fd.get('username') : modal.data.username;
                                const payload = {
                                    username: usernameInput, password: fd.get('password') || modal.data?.password || '123456',
                                    full_name: fd.get('full_name'), department: fd.get('department'), position: fd.get('position'),
                                    phone: fd.get('phone'), role: fd.get('role'), status: fd.get('status'), inspector_mapped_name: fd.get('inspector_mapped_name') || ''
                                };
                                if (window.dbFirestore) {
                                    await window.dbFirestore.collection("users").doc(usernameInput).set(payload, { merge: true });
                                }
                                setSuccessModal('บันทึกข้อมูลผู้ใช้สำเร็จ');
                                setModal(null);
                            }} className="space-y-3">
                                {modal.isNew && <div><label className="text-xs font-bold">Username</label><input type="text" name="username" required className="w-full p-2 border rounded" /></div>}
                                <div><label className="text-xs font-bold">ชื่อ-นามสกุล</label><input type="text" name="full_name" defaultValue={modal.data.full_name} required className="w-full p-2 border rounded" /></div>
                                <div><label className="text-xs font-bold">เบอร์โทร</label><input type="text" name="phone" defaultValue={modal.data.phone} className="w-full p-2 border rounded" /></div>
                                <div className="grid grid-cols-2 gap-2">
                    ...
                    {/* Modal เพิ่ม/แก้ไขผู้ตรวจ */}
                    {modal?.type === 'inspector_form' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            <button onClick={() => setModal(null)} className="absolute top-4 right-4 bg-slate-100 text-slate-500 p-2 rounded-full"><Icons.X /></button>
                            <h3 className="text-xl font-bold mb-4 border-b pb-2">{modal.data ? 'แก้ไขผู้ตรวจ' : 'เพิ่มผู้ตรวจ'}</h3>
                            <form onSubmit={async (e) => {
                                e.preventDefault();
                                const fd = new FormData(e.target);
                                ...
                                if (window.dbFirestore) {
                                    await window.dbFirestore.collection("inspectors").doc(String(name)).set({ name, product_lines: certs });
                                    if (modal.data && modal.data.name !== name) await window.dbFirestore.collection("inspectors").doc(String(modal.data.name)).delete();
                                }
                                setSuccessModal('บันทึกผู้ตรวจสำเร็จ');
                                setModal(null);
                            }}>
                                <label className="text-xs font-bold block mb-1">ชื่อผู้ตรวจ</label>
                                <input type="text" name="ins_name" defaultValue={modal.data?.name || ''} required className="w-full p-3 border rounded-xl mb-4 font-bold" />
                                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto mb-4 p-2 border rounded">
                                    {Object.keys(PRODUCT_COLORS).filter(k => k !== 'อื่นๆโปรดระบุ').map(pl => (
                                        <label key={pl} className="flex items-center gap-2 text-xs font-bold">
                                            <input type="checkbox" name="certs" value={pl} defaultChecked={modal.data?.product_lines?.includes(pl)} /> {pl}
                                        </label>
                                    ))}
                                </div>
                    ...
                    {modal?.type === 'admin_cell_action' && (
                        <div className="modal-card p-6 text-center animate-pop w-full max-w-sm bg-white rounded-3xl shadow-2xl relative">
                            <button onClick={() => setModal(null)} className="absolute top-4 right-4 bg-slate-100 text-slate-500 p-2 rounded-full"><Icons.X /></button>
                            <h3 className="text-lg font-bold text-slate-800 mb-2 border-b pb-3">จัดการคิว / วันพิเศษ</h3>
                            ...
                            <div className="space-y-3">
                                <button onClick={() => setModal({ type: 'booking', data: modal.data })} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl">จองคิวตรวจ SAIS</button>
                                <button onClick={() => { setEventStartDate(modal.data.date); setEventEndDate(modal.data.date); setModal({ type: 'manage_events' }); }} className="w-full py-3 bg-emerald-500 text-white font-bold rounded-xl">เพิ่มกิจกรรมบริษัท</button>
                                <button onClick={() => { setLeaveStartDate(modal.data.date); setLeaveEndDate(modal.data.date); setLeaveInspectors([modal.data.inspector_name]); setModal({ type: 'manage_leaves' }); }} className="w-full py-3 bg-amber-500 text-white font-bold rounded-xl">จองวันลาให้พนักงาน</button>
                                <button onClick={() => { setHolidayStartDate(modal.data.date); setHolidayEndDate(modal.data.date); setModal({ type: 'manage_holidays' }); }} className="w-full py-3 bg-red-600 text-white font-bold rounded-xl">เพิ่มวันหยุดบริษัท</button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="modal-card p-6 w-full max-w-md animate-pop bg-white rounded-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            {(isAdmin || user?.username === modal.data.created_by) && (
                                <div className="mt-6 pt-4 border-t flex flex-col gap-3">
                                    <button onClick={() => setModal({ type: 'booking', data: modal.data })} className="py-3 bg-blue-50 text-blue-700 font-bold rounded-xl border">แก้ไขข้อมูล</button>
                                    <button onClick={() => handleCancelBooking(modal.data)} className="py-3 bg-red-50 text-red-700 font-bold rounded-xl border">ลบรายการนี้</button>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {modal?.type === 'booking' && (
                        <div className="modal-card w-full max-w-[450px] animate-pop flex flex-col max-h-[90vh] bg-white rounded-3xl overflow-hidden shadow-2xl relative">
                            ...
                            <div className="p-4 border-b bg-slate-50 flex justify-between items-center pr-14">
                            ...
                            <div className="p-5 overflow-y-auto max-h-[75vh] custom-scrollbar">
                                <form onSubmit={handleBookingSubmit} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div><label className="text-xs font-bold">Product Line *</label><input type="text" name="product_line" required defaultValue={modal.data.product_line || 'ES1'} className="w-full p-2.5 border rounded-lg" /></div>
                                        <div><label className="text-xs font-bold">ประเภทงาน *</label><input type="text" name="job_type" required defaultValue={modal.data.job_type || 'New'} className="w-full p-2.5 border rounded-lg" /></div>
                                    </div>
                                    <div><label className="text-xs font-bold">พื้นที่ *</label><input type="text" name="area" required defaultValue={modal.data.area || 'กรุงเทพและปริมณฑล'} className="w-full p-2.5 border rounded-lg" /></div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div><label className="text-xs font-bold">Eq No. *</label><input type="text" name="equipment_no" required defaultValue={modal.data.equipment_no} className="w-full p-2.5 border rounded-lg" /></div>
                                        <div><label className="text-xs font-bold">Unit No. *</label><input type="text" name="unit_no" required defaultValue={modal.data.unit_no} className="w-full p-2.5 border rounded-lg" /></div>
                                    </div>
                                    <div><label className="text-xs font-bold">ชื่อโครงการ *</label><input type="text" name="site_name" required defaultValue={modal.data.site_name} className="w-full p-2.5 border rounded-lg" /></div>
                                    <div><label className="text-xs font-bold">เบอร์โทรศัพท์ *</label><input type="tel" name="tel" required maxLength="10" defaultValue={modal.data.tel} className="w-full p-2.5 border rounded-lg" /></div>
                                    
                                    <div className="bg-slate-50 p-4 rounded-xl border space-y-3">
                                        <h4 className="text-sm font-bold border-b pb-2">อัปโหลดเอกสาร (Layout, Wiring, Precheck)</h4>
                                    ...
                                    <button type="submit" disabled={loadingMsg} className="w-full py-4 rounded-xl font-bold text-white bg-red-600 shadow-lg mt-6">
            ...
            {/* Global Modal สำหรับเปิดดูรูปภาพ/ไฟล์ */}
            {viewFileUrl && (
            ...
            {alertMsg && (
                <div className="backdrop z-[500] p-4 flex items-center justify-center">
                    <div className="bg-white w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl animate-pop">
                        <h3 className="text-lg font-bold text-slate-800 mb-2">แจ้งเตือน</h3>
                        <p className="text-sm text-slate-600 mb-6 whitespace-pre-line">{alertMsg}</p>
                        <button onClick={() => setAlertMsg(null)} className="w-full py-3 bg-slate-800 text-white font-bold rounded-xl">ตกลง</button>
                    </div>
                </div>
            )}
                ...
                <div className="backdrop z-[600] p-4 flex items-center justify-center">
                    <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-pop">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setConfirmDialog(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl">ยกเลิก</button>
                            <button onClick={confirmDialog.onConfirm} className="flex-1 py-3 bg-red-600 text-white font-bold rounded-xl">ยืนยัน</button>
                        </div>
                    </div>
                </div>
            )}
