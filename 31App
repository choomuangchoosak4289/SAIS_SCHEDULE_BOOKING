...
const App = () => {
    ...
    // 📍 1. ประกาศตัวแปร State ทั้งหมด (เรียงลำดับอย่างถูกต้องเพื่อป้องกัน Error หน้าจอขาว 100%)
    ...
    // 📍 2. โหลด Firebase Compat ผ่าน Script Tag (เสถียร 100% บน Apps Script ไม่ทำหน้าจอขาว)
    useEffect(() => {
    ...
    // 📍 3. ฟังก์ชันย้ายฐานข้อมูลจาก Sheets ไป Firestore (รันครั้งเดียว แบ่งอัปโหลดเป็น Batch กันค้าง)
    const handleMigrateToFirestore = async () => {
        ...
        try {
            ...
            let batch = window.dbFirestore.batch();
            let batchCount = 0;
            
            for (let i = 0; i < bookings.length; i++) {
                const b = bookings[i];
                if (!b.id) continue;
                const docRef = window.dbFirestore.collection("bookings").doc(String(b.id));
                batch.set(docRef, b);
                bookingCount++;
                batchCount++;
                ...
                if (batchCount >= 400) {
                    await batch.commit();
                    batch = window.dbFirestore.batch();
                    batchCount = 0;
                }
            }
            ...
            for (let i = 0; i < users.length; i++) {
                ...
                const docRef = window.dbFirestore.collection("users").doc(String(u.username));
                batch.set(docRef, u);
                userCount++;
                batchCount++;
                if (batchCount >= 400) {
            ...
            for (let i = 0; i < inspectors.length; i++) {
                const ins = inspectors[i];
                if (!ins.name) continue;
                const docRef = window.dbFirestore.collection("inspectors").doc(String(ins.name));
                batch.set(docRef, ins);
                inspectorCount++;
                batchCount++;
                if (batchCount >= 400) {
                    await batch.commit();
    ...
    // 📍 ระบบ API Action แบบลื่นไหล ไม่กวนหน้าจอตอนทำงาน (Background Sync)
        ...
        try {
            ...
            try {
                ...
                if (result.status === 'ok') { 
                    ...
                    if(customLoadMsg) setLoadingMsg(null);
                    // 📍 ซ่อนข้อความแจ้งเตือนหา ID ไม่เจอ (เพื่อไม่ให้ผู้ใช้สับสนระหว่างย้ายข้อมูลมา Firebase)
    ...
    const handleBulkDelete = async (type, ids) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                // 📍 ลบออกจาก Firebase โดยตรงแบบ Real-time
                ...
                // 📍 แบ็คอัปคำสั่งลบไปที่ Google Sheets (เบื้องหลัง ไม่กวนหน้าจอ)
    ...
    // 📍 ระบบแจ้งเตือน (กรองเฉพาะผู้ใช้ปัจจุบัน)
    ...
    // 📍 3. ดึงข้อมูล Realtime ด้วย Firestore + Hybrid Fallback (100% แก้ปัญหาผู้ใช้งานใหม่)
    useEffect(() => {
        let unsubscribe;
        if (user && isFirebaseReady && window.dbFirestore) {
            unsubscribe = window.dbFirestore.collection("bookings").onSnapshot((snapshot) => {
                const firestoreBookings = [];
                snapshot.forEach(doc => firestoreBookings.push(doc.data()));
                
                // 📍 Hybrid Logic: ถ้าใน Firebase มีข้อมูลแล้ว ให้ใช้จาก Firebase เป็นหลัก
                // แต่ถ้า Firebase ว่าง (เพิ่งเข้าใช้งาน ยังไม่ย้ายข้อมูล) จะยังไม่ทับข้อมูลจาก Sheets
            ...
            // ดึงข้อมูลระบบอื่นๆ (รายชื่อ, แจ้งเตือน, ตั้งค่า) แบบเงียบๆ
            fetchCoreData(true, null); 
        } else if (user) {
            // ถ้า Firebase พังจริงๆ ให้ดึงจาก Sheets เป็นตัวเลือกสุดท้าย
            fetchCoreData(true, null);
        }
    ...
    // 📍 Smart Polling ตรวจจับว่าผู้ใช้อยู่หน้าจอหรือไม่ เพื่อหยุดดึงข้อมูลประหยัดเซิร์ฟเวอร์
    useEffect(() => {
        const load = async () => {
    ...
    const fetchCoreData = async (needPast = false, currentCache = null) => {
        ...
        try {
            ...
            if (result.status === 'ok') {
                ...
                // 📍 อัปเดตข้อมูลรอง แต่ถ้า bookings มีใน Firebase แล้ว จะไม่ให้ Sheets ทับ (หัวใจของ Hybrid)
                setDb(prev => ({ 
                    ...prev, 
                    inspectors: finalDb.inspectors || prev.inspectors,
                    notifications: finalDb.notifications || prev.notifications,
                    settings: finalDb.settings || prev.settings,
                    // ถ้าระบบเพิ่งรันและยังไม่มีข้อมูล Firebase (length = 0) ให้ใช้จาก Sheets ไปก่อน
                    bookings: finalDb.bookings && finalDb.bookings.length > 0 && prev.bookings.length === 0 ? finalDb.bookings : prev.bookings
                }));
    ...
    const handleTabChange = (view) => {
        setCurrentView(view);
        // 📍 โหลดข้อมูลสถิติเสมอเมื่อเปลี่ยนแท็บ (เผื่ออัปเดตข้อมูล Admin / ผู้ใช้งาน)
    ...
    const handleCancelBooking = (booking) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                setModal(null); // ปิดโมดอลทันที ไม่ต้องรอโหลด
                ...
                try {
                    ...
                    // ลบจาก Firebase ทันทีแบบเดา ID อัจฉริยะ
                    ...
                    // สั่ง Backup ไป Google Sheets (เบื้องหลัง)
                ...
                } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการลบ'); setLoadingMsg(null); }
            }
        });
    };
        ...
        setPromptDialog({
            ...
            onSubmit: async (reason) => {
                ...
                try {
                    ...
                    apiAction({ action: 'delete_booking', id: docId || booking.id || '', user: user?.username || 'admin', reason: logDetail, job_type: booking.job_type, equipment_no: booking.equipment_no }, null, true);
                } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการยกเลิก'); setLoadingMsg(null); }
            }
        });
    };
    ...
    const handleVerifyDoc = async (booking, docField, isChecked) => {
        ...
        } catch(e) { setAlertMsg('อัปเดตสถานะไม่สำเร็จ'); setLoadingMsg(null); }
    };
    ...
    const handleFileUpload = async (e, docType, isMultiple = false) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // 📍 ฟังก์ชันอัปโหลดภาพยังต้องยิงไป Apps Script เพื่อแปลงลง Drive เหมือนเดิม
    ...
    const handleTrashDrop = async (e) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการลบ'); setLoadingMsg(null); }
            }
        });
    };
    ...
    const handleDrop = async (e, targetDate, targetInspector) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการย้าย'); setLoadingMsg(null); }
            }
        });
    };
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        try {
            ...
            apiAction({ ...payload, id: docId || modal.data.id }, null, true);
        } catch (e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาด'); }
    };
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            } catch(e) { console.error(e); setAlertMsg('ข้อผิดพลาดในการบันทึก'); setLoadingMsg(null); }
            return;
        }
        ...
        try {
            ...
            // 📍 เขียนลง Firebase ทันทีแบบไม่ต้องรอ
            if (window.dbFirestore) {
            ...
            // 📍 ส่งเข้า Sheets เบื้องหลัง
            apiAction(payload, null, true);
        } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการบันทึก'); setLoadingMsg(null); }
    };
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                try {
                                    ...
                                    if (!isFound) {
                                        ...
                                        const result = await res.json();
                                        if (result.status === 'ok') { 
                                            // นำข้อมูลกลับมาบันทึกลง Firebase ด้วย
                            ...
                            // 📍 โหมดสมัครสมาชิกใหม่แบบ Hybrid
                                ...
                                try {
                                    ...
                                    // เช็ค Firebase ก่อนว่ามีชื่อซ้ำหรือไม่
                                    if (window.dbFirestore) {
                                    ...
                                    // ส่งไป Backup ที่ Google Sheets เบื้องหลัง
                            ...
                            // 📍 โหมดล็อกอินเข้าสู่ระบบ (Hybrid System แก้ปัญหา Firebase ว่างเปล่า)
                            else {
                                ...
                                try {
                                    ...
                                    // 1. ลองค้นหาใน Firebase ก่อน (เพื่อความรวดเร็ว)
                                    if (window.dbFirestore) {
                                    ...
                                    // 2. ถ้าใน Firebase ยังไม่มีข้อมูล (เพิ่งติดตั้งใหม่ หรือยังไม่ได้ Migrate) ให้ค้นหาผ่าน Google Sheets
                                    if (!userData && SCRIPT_URL) {
                                        ...
                                        if (result.status === 'ok') {
                                            ...
                                            // คัดลอกข้อมูลผู้ใช้คนนี้ไปเขียนลง Firebase ทันที เพื่อให้การล็อกอินครั้งต่อไปเร็วขึ้น
                                            if (window.dbFirestore) {
        ...
        <div className="app-container">
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view relative pb-20">
                    ...
                    {/* 📍 Tab ปรับแต่งเว็บไซต์ อัปเกรดให้ใช้ปุ่ม - (ตัวเลข) + เพื่อความง่าย */}
                        ...
                        <div className="space-y-4 animate-pop pb-10">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    apiAction({ action: 'save_settings', settings: settingsObj }, null, true);
                                } catch(err) {
                                ...
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2 hover:border-blue-300 transition-all">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        ...
                                        <button onClick={() => {
                                            ...
                                            if (window.dbFirestore) window.dbFirestore.collection("users").doc(u.username).update({ status: newStatus });
                                            apiAction({action: 'update_user_status', target_user: u.username, new_status: newStatus, admin_user: user?.username}, null, true);
                                        ...
                                        <button onClick={() => {
                                            setConfirmDialog({
                                                ...
                                                onConfirm: async () => {
                                                    ...
                                                    apiAction({action: 'delete_user', target_user: u.username, admin_user: user?.username}, null, true);
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => setConfirmDialog({msg:`ยืนยันการลบผู้ตรวจ "${ins.name}" ออกจากระบบ?`, onConfirm: async() => {
                                                    ...
                                                    apiAction({action: 'manage_inspector', sub_action: 'delete', old_name: ins.name}, null, true);
                                                    setSuccessModal('ลบสำเร็จ');
                    ...
                    {modal?.type === 'edit_user' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.isNew) {
                                    setLoadingMsg('กำลังสร้างบัญชีใหม่...');
                                    const usernameInput = fd.get('username');
                                    if (window.dbFirestore) {
                                        const docSnap = await window.dbFirestore.collection("users").doc(usernameInput).get();
                                        if (docSnap.exists) {
                                    ...
                                    apiAction({ ...payload, action: 'update_user_details', target_user: modal.data.username, admin_user: user?.username }, null, true);
                        ...
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    apiAction({ action: 'manage_inspector', sub_action: modal.data ? 'update' : 'add', old_name: modal.data?.name || '', new_name: name, product_lines: certs, user: user?.username }, null, true);
                                    
                                    setLoadingMsg(null);
                                    setSuccessModal('บันทึกข้อมูลผู้ตรวจสำเร็จ'); 
                                    setModal(null);
                                } catch(e) {
                                    setLoadingMsg(null);
                        ...
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.data?.id) {
                                    ...
                                    setLoadingMsg(null);
                                    setSuccessModal('อัปเดตวันลาสำเร็จ'); 
                                    setModal(null);
                                    
                                    apiAction({ action: 'update_booking', id: docId || modal.data.id, date: sDate, site_name: lType, user: user?.username, reason: 'แก้ไขวันลาส่วนตัว' }, null, true);
                                } else {
                                    ...
                                    setLoadingMsg(null);
                                    setSuccessModal('แจ้งวันลาสำเร็จ'); 
                                    setModal(null);
                                    
                                    apiAction({ action: 'create_multiple_bookings', dates: dates, inspector_name: targetName, job_type: 'leave', site_name: lType, equipment_no: eqNo, user: user?.username, reason: 'แจ้งวันลาส่วนตัว' }, null, true);
                                }
                            }} className="space-y-4">
                                {modal.data?.id ? (
                                    <div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            for (let target of targets) {
                                                ...
                                                // Sync หลังบ้านแบบไร้รอยต่อ
                                                apiAction({ action: 'create_multiple_bookings', dates: leaveDates, inspector_name: target, job_type: 'leave', site_name: finalType, equipment_no: eqNo, user: user?.username, reason: logDetail }, null, true);
                                            }
                                            
                                            setLoadingMsg(null);
                                            setSuccessModal('เพิ่มวันลาสำเร็จ'); 
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-100">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            // ถ้าเลือกทุกคน ให้เป้าหมายคือ SYSTEM_EVENT ถ้าไม่ ให้วนลูปสร้างให้แต่ละคน
                                            let targets = eventInspectors.includes('ALL') ? ['SYSTEM_EVENT'] : eventInspectors;
                                            
                                            setLoadingMsg('กำลังสร้างกิจกรรม...');
                                            for (let target of targets) {
                                                // ฝังรหัสสีไปใน Eq No เพื่อให้หน้าตารางดึงสีมาแสดงผล (เช่น EVENT_12345_#ff0000)
                                                const eqNo = `EVENT_${Date.now()}_${eventColor}`;
                                                const logDetail = `[สร้างกิจกรรม]\nโดย: ${user?.username}\nเรื่อง: ${eName}\nผู้เข้าร่วม: ${target}`;
                                                ...
                                                // Sync หลังบ้านแบบเงียบๆ
                                                apiAction({ action: 'create_multiple_bookings', dates: eventDates, inspector_name: target, job_type: 'company_event', site_name: eName, equipment_no: eqNo, user: user?.username, reason: logDetail }, null, true);
                                            }
                                            
                                            setLoadingMsg(null);
                                            setSuccessModal('สร้างกิจกรรมสำเร็จ');
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-sm border">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            apiAction({ action: 'create_multiple_bookings', dates: holidayDates, inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: hName, equipment_no: eqNo, user: user?.username, reason: 'เพิ่มวันหยุด' }, null, true);
                                            
                                            setLoadingMsg(null);
                                            setSuccessModal('สร้างวันหยุดสำเร็จ'); 
