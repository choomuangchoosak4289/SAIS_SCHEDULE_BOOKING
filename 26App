    ...
    return (
        <div id="calendar-export-area" className={`calendar-grid ${isExporting ? 'export-mode' : ''}`} style={{ 
            ...
            {daysInView.map((d, index) => {
                // 📍 แก้ไข: กำหนดเฉพาะให้ตัวหนังสือสีแดงสำหรับวันอาทิตย์ ไม่เปลี่ยนสีพื้นหลัง เพื่อป้องกันการเลอะทะลุ
                            ...
                            return (
                                <div key={idx} 
                                    ...
                                    onClick={() => {
                                        ...
                                        if (user.role === 'viewer') return; // 📍 สิทธิ์ Viewer คลิกลงตารางไม่ได้
...
const App = () => {
    ...
    // 💡 availableInspectors มีการเรียกใช้ modal.data?.username จึงต้องประกาศ modal ก่อนหน้านี้ (แก้ไขแล้ว 100%)
    ...
    const handleBookingSubmit = async (e) => {
        ...
        } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการบันทึก'); }
    };
