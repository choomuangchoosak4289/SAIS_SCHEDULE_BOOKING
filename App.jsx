...
const CalendarGrid = React.memo(({ daysInView, db, isAdmin, user, setModal, setAlertMsg, handleDrop, handleDragOver, handleDragLeave, handleDragStart, handleDragEnd, setConfirmDialog, apiAction, setQuickAddType, filteredBookings, tableFontScale, columnZoom, specialFontScale, isExporting }) => {
    ...
    // ฟังก์ชันจัดการอัปเดตสถานะเอกสารสำหรับ Admin
    ...
    // ฟังก์ชันดาวน์โหลดไฟล์สำหรับ Admin/Inspector
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // Admin ต้องมาติ๊กเอง User ส่งไปจะเป็น false ไปก่อน ยกเว้นแต่มีไฟล์อัพโหลดให้เป็น false ไว้รอตรวจ
        if (!modal?.data?.id) {
    ...
    // บล็อก UI สำหรับหน้า Login โดยเฉพาะ (ป้องกันข้อมูลตารางทั้งหมด)
    if (!user) {
        return (
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                const res = await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'reset_password', full_name: fd.get('full_name'), phone: fd.get('phone'), new_password: fd.get('password') }) });
                                const result = await res.json();
                                setLoadingMsg(null);
                                if (result.status === 'ok') { 
                                ...
                                try {
                                    const result = await utils.fetchWithRetry(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'login', username: fd.get('username'), password: fd.get('password') }) });
                                    setLoadingMsg(null);
                                    if (result.status === 'ok') { 
    ...
    return (
        <div className="app-container">
            ...
            {/* ส่วนที่เหลือยังคงครอบคลุมหน้าจอ Detail/Booking/Modal ตามเดิม... */}
            {modal?.type === 'detail' && (
