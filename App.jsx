...
const CalendarGrid = React.memo(({ daysInView, db, isAdmin, user, setModal, setAlertMsg, handleDrop, handleDragOver, handleDragLeave, handleDragStart, handleDragEnd, setConfirmDialog, setQuickAddType, filteredBookings, tableFontScale, columnZoom, specialFontScale, isExporting }) => {
                            ...
                            return (
                                <div key={idx} 
                                    ...
                                    onClick={() => {
                                        ...
                                        // 📍 แก้บัค Infinite Loop: แยกเงื่อนไขและเรียก Modal ให้ชัดเจน
                                        if (isAdmin) {
                                    ...
                                    {d.isGlobalEvent && !hasLeave && d.globalEvents.map((ge, gei) => {
                                        const isCard = cellTasks.length > 0;
                                        // 📍 ดึงสีจาก Equipment No. (EVENT_timestamp_#color) ถ้ามี
                                        let customColor = db.settings?.eventBg || '#22c55e';
...
const App = () => {
    ...
    // State สำหรับเปิดดูรูปภาพ/เอกสาร และเก็บ URL ที่อัปโหลด
    ...
    // ตัวแปรสำหรับจัดการวันหยุดและกิจกรรม
    ...
    // 📍 2. โหลด Firebase Compat (เฉพาะ Firestore)
    useEffect(() => {
    ...
    // 📍 3. ดึงข้อมูล Realtime ด้วย Firestore 100% แบบ onSnapshot
    useEffect(() => {
        ...
        // 3.3 ตรวจจับคิวงานทั้งหมด (Bookings) - เปลี่ยนระบบเป็น Realtime
        ...
        // 3.4 แจ้งเตือน
        const unsubNotifs = window.dbFirestore.collection("notifications").orderBy("timestamp", "desc").limit(50).onSnapshot(snapshot => {
            const list = [];
            snapshot.forEach(doc => list.push(doc.data()));
        ...
        // 3.5 ข้อมูลสำหรับ Admin เท่านั้น
        if (user && user.role === 'admin') {
            ...
            unsubLogs = window.dbFirestore.collection("logs").orderBy("timestamp", "desc").limit(200).onSnapshot(snapshot => {
                const list = [];
                snapshot.forEach(doc => list.push(doc.data()));
        ...
        return () => {
            unsubSettings(); unsubInspectors(); unsubBookings(); unsubNotifs();
            if(unsubUsers) unsubUsers();
            if(unsubLogs) unsubLogs();
        };
    }, [isFirebaseReady, user]);

    // 📍 ฟังก์ชันบันทึกประวัติการทำงานลง Firestore
    ...
    // 📍 ฟังก์ชันอัปโหลดไฟล์ไปที่ Google Drive (ผ่าน Google Apps Script)
    const handleFileUpload = async (e, docType) => {
        ...
        try {
            ...
            // แปลงไฟล์เป็น Base64 (ถ้าเป็นรูปให้บีบอัดก่อน)
            if (mimeType.startsWith('image/')) {
                base64Data = await window.SAIS_UTILS.compressImage(file);
                ...
                throw new Error("ระบบรองรับเฉพาะไฟล์รูปภาพและ PDF เท่านั้นครับ");
            }

            // ตัดส่วนหัว base64 ออกให้เหลือแค่ data เพียวๆ
            const base64Clean = base64Data.split(',')[1] || base64Data;

            // ส่งข้อมูลไปยัง Google Apps Script
            const payload = {
                action: "upload_image",
                api_key: window.SAIS_CONFIG?.API_KEY || "SAIS_SECURE_KEY_2026",
                fileName: `${docType}_${Date.now()}_${file.name.replace(/[^a-zA-Z0-9.]/g, '')}`,
                mimeType: mimeType,
                base64: `data:${mimeType};base64,${base64Clean}` // ส่งรูปแบบเต็มไปให้ฝั่ง GAS แงะ
            };

            const response = await fetch(window.SAIS_CONFIG.SCRIPT_URL, {
                method: "POST",
            ...
            if (result.status === 'ok') {
                setDocUrls(prev => ({ ...prev, [docType]: result.fileUrl }));
                setSuccessModal(
                    <div className="text-center">
                        <div className="font-black text-sm mb-1 text-green-600">อัปโหลดไฟล์สำเร็จ!</div>
                        <div className="text-xs text-slate-600">ไฟล์ถูกเก็บใน Google Drive เรียบร้อยแล้ว</div>
                    </div>
                );
            } else {
    ...
    // 📍 ฟังก์ชันจัดการสีของธีมแบบ Dynamic
    const DynamicStyles = () => {
        const s = db.settings || {};
        return (
            <style>{`
    ...
    // 📍 ฟังก์ชันเปรียบเทียบการเปลี่ยนแปลงเพื่อเก็บ Log
    const getDiffLog = useCallback((oldData, newData, actionUser) => {
        ...
        let changes = [];
        const labels = {
            date: 'วันที่', inspector_name: 'ผู้ตรวจ', site_name: 'โครงการ',
            equipment_no: 'Eq No.', unit_no: 'Unit', job_type: 'ประเภทงาน', 
            area: 'พื้นที่', tel: 'เบอร์โทร', product_line: 'Product',
            layout_doc: 'Layout', wiring_doc: 'Wiring', precheck_doc: 'Precheck'
        };
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    setLoadingMsg(null);
                    setSuccessModal(`ลบข้อมูลสำเร็จจำนวน ${ids.length} รายการ`);
                } catch(e) {
                    setLoadingMsg(null);
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (window.dbFirestore && task.id) {
                        await window.dbFirestore.collection("bookings").doc(String(task.id)).delete();
                    }
                    logActivity('DELETE VIA TRASH', `[ลบรายการด้วย Drag & Drop]\nรายการ: ${task.site_name || task.equipment_no}`);
                    setLoadingMsg(null);
                    setSuccessModal('ทิ้งรายการลงถังขยะสำเร็จ!');
                } catch(e) { 
                    setAlertMsg('เกิดข้อผิดพลาดในการลบทิ้ง: ' + e.message); 
                    setLoadingMsg(null); 
                }
            }
        });
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (window.dbFirestore && task.id) {
                        await window.dbFirestore.collection("bookings").doc(String(task.id)).update({ 
                            date: targetDate, 
                    ...
                    setAlertMsg('เกิดข้อผิดพลาดในการย้าย: ' + e.message); 
                    setLoadingMsg(null); 
                }
            }
        });
    ...
    // 📍 ระบบลบ/ยกเลิก (Firebase Direct)
    const handleCancelBooking = (booking) => {
        if(!booking?.id && !booking?.equipment_no) return;
        setConfirmDialog({
            msg: "ยืนยันลบข้อมูลนี้ใช่หรือไม่?",
            onConfirm: async () => {
                setConfirmDialog(null);
                setModal(null); // ปิด Modal ทันทีลด UI Delay
                ...
                try {
                    ...
                    logActivity('DELETE BOOKING', `[ลบรายการ]\nโครงการ: ${booking.site_name || '-'}\nวันที่: ${formatSafeDate(booking.date)}`);
                    setLoadingMsg(null);
        ...
        setPromptDialog({
            ...
            onSubmit: async (reason) => {
                ...
                try {
                    ...
                    logActivity('CANCEL BOOKING', `[ยกเลิกคิวงาน]\nโครงการ: ${booking.site_name || '-'}\nEq No.: ${booking.equipment_no || '-'}\nเหตุผล: ${reason || 'ไม่ระบุ'}`);
                    setLoadingMsg(null);
                    setSuccessModal(`ยกเลิกคิวงานสำเร็จ!\nเหตุผล: ${reason}`);
                } catch(e) { 
                    setAlertMsg('เกิดข้อผิดพลาดในการยกเลิก: ' + e.message); 
                    setLoadingMsg(null); 
                }
            }
        });
    ...
    // 📍 ระบบจัดการบันทึกข้อมูล (CRUD Firebase 100%)
    const handleEditSpecialSubmit = async (e) => {
        ...
        try {
            ...
            if (docId && window.dbFirestore) {
                await window.dbFirestore.collection("bookings").doc(docId).update({ 
                    site_name: newTitle, inspector_name: newInspector, date: newDate 
                });
            }
            logActivity('EDIT SPECIAL', `[แก้ไขข้อมูลพิเศษ]\nหัวข้อ: ${newTitle}\nวันที่: ${newDate}\nผู้ตรวจ: ${newInspector}`);
            setLoadingMsg(null);
            setSuccessModal('อัปเดตข้อมูลพิเศษสำเร็จ!');
        } catch (e) { 
            setAlertMsg('เกิดข้อผิดพลาด: ' + e.message); 
            setLoadingMsg(null); 
        }
    };
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            try {
                ...
                const docId = modal?.data?.id || generateUniqueId();
                setModal(null); // ปิด Modal ทันที
                
                if (window.dbFirestore) {
                    await window.dbFirestore.collection("bookings").doc(String(docId)).set({
                        id: docId, 
                        date: targetDate, 
                        inspector_name: quickAddType === 'holiday' ? 'SYSTEM_HOLIDAY' : targetInspector,
                        job_type: p_jobType, 
                ...
                setLoadingMsg(null);
                setSuccessModal('บันทึกข้อมูลสำเร็จ!');
        ...
        const payload = {
            ...
            date: targetDate, 
            user: user?.username, 
            created_by: modal?.data?.created_by || user?.username,
        ...
        try {
            ...
            if (window.dbFirestore) {
                await window.dbFirestore.collection("bookings").doc(String(docId)).set({ ...payload, status: 'active' }, { merge: true });
            }
            ...
            if (isAdmin && !modal?.data?.id && fd.get('keep_open')) {
                ...
                setModal(null);
                setAreaSelection(''); setJobTypeSelection(''); setProductLineSelection('');
                setDocUrls({ layout: '', wiring: '', precheck: '', site_cond_1: '', site_cond_2: '', site_cond_3: '', site_cond_4: '', site_cond_5: '', site_cond_6: '' });
            }
        } catch(e) { 
            console.error(e); 
            setAlertMsg('เกิดข้อผิดพลาดในการบันทึก: ' + e.message); 
            setLoadingMsg(null); 
        }
    };
    const handleLogout = () => {
    ...
    // 📍 5. หน้าจอ Login (ระบบ Firebase 100% ถอด Google Sheets ออกอย่างสมบูรณ์)
    if (!user) {
        return (
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                setLoadingMsg('กำลังค้นหาบัญชีบนฐานข้อมูล...');
                                try {
                                    if (window.dbFirestore) {
                                        const snapshot = await window.dbFirestore.collection("users")
                                            .where("full_name", "==", fd.get('full_name'))
                                            .where("phone", "==", fd.get('phone')).get();
                            ...
                            // 📍 โหมดสมัครสมาชิก
                                ...
                                setLoadingMsg('กำลังบันทึกข้อมูลสมัครสมาชิก...');
                                try {
                                    ...
                                    if (window.dbFirestore) {
                                        ...
                                        if (userDoc.exists && userDoc.data().password === passwordInput) {
                                            ...
                                            if (userData.status === 'blocked') {
                                                ...
                                                delete userData.password; // ลบรหัสผ่านทิ้งเพื่อความปลอดภัย
                            ...
                            {isRegisterMode && (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div><label className="text-[10px] font-bold text-slate-500">Username</label><input name="username" required placeholder="ตั้ง Username" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm font-bold" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500">เบอร์โทรศัพท์</label><input name="phone" required placeholder="08XXXXXXXX" maxLength="10" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm font-bold" onInput={(e) => { e.target.value = e.target.value.replace(/[^0-9]/g, ''); }} /></div>
                                    </div>
                                    <div><label className="text-[10px] font-bold text-slate-500">ชื่อ-นามสกุล (จริง)</label><input name="full_name" required placeholder="ชื่อ นามสกุล" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm font-bold" /></div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <div><label className="text-[10px] font-bold text-slate-500">แผนก</label><input type="text" name="department" required placeholder="NI , MOD" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500">ตำแหน่ง</label><input type="text" name="position" required placeholder="PE,PM" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm" /></div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                            ...
                            {isForgotMode && (
                                <>
                                    <div className="bg-amber-50 p-3 rounded-xl mb-2 text-[11px] text-amber-800 border border-amber-200">ระบุ <b>ชื่อ-นามสกุล</b> และ <b>เบอร์โทรศัพท์</b> ให้ตรงกับตอนสมัคร เพื่อรีเซ็ตรหัสผ่านใหม่</div>
        ...
        <div className="app-container">
                ...
                <div className="fixed inset-0 z-[700] flex items-center justify-center pointer-events-none p-4">
                    <div className="bg-white w-full max-w-[320px] rounded-3xl p-6 shadow-2xl animate-pop border-4 border-green-400">
                        ...
                        {typeof successModal === 'string' ? (
                            ...
                            successModal // เรนเดอร์ Element HTML ที่ส่งเข้ามา
                        )}
                    </div>
                </div>
            )}
                ...
                <div className="grid-container relative overflow-hidden pb-16">
                    <div className="nav-bar bg-white px-3 py-2 border-b flex-shrink-0 z-[45]">
                        <div className="flex justify-between items-center w-full">
                            <button onClick={() => changePeriod('prev')} className="px-3 py-1.5 bg-slate-100 rounded-lg text-xs font-bold text-slate-600 active:scale-95 transition-transform"><Icons.ChevronLeft /> ย้อน</button>
                            <div className="text-center font-bold text-slate-800 text-sm">
                                ...
                                {currentDate.toLocaleDateString('th-TH', { timeZone: 'Asia/Bangkok', month: 'short', year: 'numeric' })}
                            </div>
                            <button onClick={() => changePeriod('next')} className="px-3 py-1.5 bg-slate-100 rounded-lg text-xs font-bold text-slate-600 active:scale-95 transition-transform">ถัดไป <Icons.ChevronRight /></button>
                        </div>
                    </div>
                    ...
                    <div className="grid-wrapper" ref={scrollRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ transform: `translateY(${pullY}px)`, transition: pullY === 0 ? 'transform 0.3s ease-out' : 'none' }}>
                        {initialLoad ? (
                            <div className="w-full h-full p-4"><div className="w-full h-16 skeleton rounded-lg bg-slate-100 animate-pulse"></div></div>
                        ) : (
                            <CalendarGrid 
            ...
            {currentView === 'documents' && isAdmin && (
                <div className="page-view relative pb-20">
                    <div className="sticky top-0 bg-[#f1f5f9] z-10 pb-4 pt-2 border-b border-slate-200">
                        ...
                        <div className="text-xs text-slate-500 mt-1">คลิกที่ Checkbox เพื่อยืนยันว่าได้รับและตรวจสอบเอกสารแล้ว (Realtime)</div>
                    </div>
                    <div className="space-y-4 pb-10 pt-4">
                        {(() => {
                            ...
                            if (docTasks.length === 0) return <div className="text-center text-slate-400 p-8 border-2 border-dashed border-slate-200 rounded-xl mt-4">ไม่มีรายการคิวงานให้ตรวจเอกสาร</div>;
                            
                            return docTasks.slice(0, 50).map((h, i) => {
                                ...
                                return (
                                    <div key={i} className={`bg-white p-4 rounded-xl shadow-sm border-2 transition-all ${all_ok ? 'border-green-200 bg-green-50/30' : 'border-slate-200'}`}>
                                        <div className="flex justify-between items-start mb-3 border-b border-slate-100 pb-2">
                                            <div className="cursor-pointer flex-1 pr-2" onClick={() => setModal({ type: 'detail', data: h })}>
                                                ...
                                                <div className="text-[10px] text-slate-500 mt-0.5 truncate">{h.site_name}</div>
                                            </div>
                                            <div className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-1 rounded flex-shrink-0">{h.date ? formatSafeDate(h.date) : ''}</div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2">
                                            {['layout', 'wiring', 'precheck'].map(docKey => {
                                                ...
                                                return (
                                                    <div key={docKey} className={`flex flex-col gap-1 items-center p-2 rounded-lg border ${isSent ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'}`}>
                                                        <div className={`text-[10px] font-bold uppercase mb-1 ${isSent ? 'text-green-700' : 'text-slate-600'}`}>{docKey}</div>
                                                        <label className="flex items-center gap-1 cursor-pointer">
                                                            <input type="checkbox" checked={isSent} onChange={async (e) => {
                                                                // 📍 จัดการ Firebase อัปเดตสถานะเอกสารโดยตรงแบบไร้รอยต่อ
                                                                const isChecked = e.target.checked;
                                                                const val = isChecked ? 'true' : 'pending';
                                                                const docName = docKey.toUpperCase();
                                                                try {
                                                                    setLoadingMsg(`กำลังอัปเดตสถานะ ${docName}...`);
                                                                    const docId = await findDocIdFallback(h);
                                                                    if (docId && window.dbFirestore) {
                                                                        await window.dbFirestore.collection("bookings").doc(docId).update({ [`${docKey}_doc`]: val });
                                                                    }
                                                                    logActivity('VERIFY DOCUMENT', `[อัปเดตเอกสาร]\nเอกสาร: ${docName} -> ${isChecked ? 'ตรวจสอบแล้ว' : 'รอตรวจสอบ'}\nโครงการ: ${h.site_name}`);
                                                                    setLoadingMsg(null);
                                                                    // ให้แสดง Popup สั้นๆ หรือไม่แสดงเลยก็ได้เพื่อความลื่นไหล แต่เพื่อความชัวร์ให้โชว์
                                                                    setSuccessModal(`อัปเดต ${docName} สำเร็จ!`);
                                                                } catch(err) {
                                                                    setAlertMsg('อัปเดตสถานะไม่สำเร็จ: ' + err.message);
                                                                    setLoadingMsg(null);
                                                                }
                                                            }} className="w-3.5 h-3.5 accent-blue-600 cursor-pointer shadow-sm" />
                                                            <span className={`text-[9px] font-bold ${isSent ? 'text-green-600' : 'text-slate-500'}`}>{isSent ? 'ตรวจแล้ว' : 'รอตรวจสอบ'}</span>
                                                        </label>
                                                        <div className="flex w-full gap-1 mt-1">
                                                            {fileUrl ? (
                                                                <button onClick={() => setViewFileUrl(fileUrl)} className="flex-1 text-[9px] bg-blue-600 text-white border border-blue-600 rounded py-1 font-bold shadow-sm active:scale-95 transition-transform">ดูไฟล์</button>
                                                            ) : (
                                                                <div className="flex-1 text-[9px] bg-slate-100 text-slate-400 border border-slate-200 rounded py-1 font-bold text-center">ไม่มีไฟล์</div>
                                                            )}
                                                        </div>
                                                    </div>
                                                );
                ...
                <div className="page-view relative pb-20">
                    <div className="sticky top-0 bg-[#f1f5f9] z-10 pb-4 pt-2">
                        <div className="flex justify-between items-center mb-4 gap-2">
                            ...
                            <div className="flex gap-2 w-full justify-end">
                                <select className="text-[11px] border border-slate-300 rounded-lg p-2 bg-white outline-none w-24 shadow-sm font-bold text-slate-600" value={filterArea} onChange={(e) => setFilterArea(e.target.value)}>
                                    ...
                                    <option value="กรุงเทพและปริมณฑล">กทม.</option>
                        ...
                        <div className="flex items-center bg-white border border-slate-300 rounded-xl px-3 py-3 shadow-sm focus-within:border-blue-400 transition-colors">
                            ...
                            {localSearchQuery && <button onClick={() => { setLocalSearchQuery(''); setSearchQuery(''); }} className="text-slate-400 p-1 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors"><Icons.X /></button>}
                        </div>
                    </div>
                    <div className="space-y-3 pb-10">
                        {/* ป้องกัน Admin หาข้อมูลไม่เจอตอนโหลด */}
                        {!hasLoadedAdmin && isAdmin ? (
                            ...
                            (() => {
                                // 📍 Admin ใช้ all_bookings เพื่อให้หาเจอทุกอย่าง | ส่วน User ใช้ bookings ทั่วไป
            ...
            {currentView === 'my_bookings' && !isAdmin && user?.role !== 'viewer' && (
                <div className="page-view relative pb-20 animate-pop">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => setShowRoleHelp(!showRoleHelp)} className="text-[10px] font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-200 flex items-center gap-1 active:scale-95 transition-transform">
                    ...
                    {showRoleHelp && (
                        <div className="bg-slate-800 text-white p-4 rounded-xl mb-4 shadow-md text-xs leading-relaxed animate-pop relative">
                            ...
                            {user?.role === 'inspector' ? (
                                ...
                                <ul className="list-disc pl-4 space-y-1.5 text-slate-200">
                                    <li><b>⏳ รอดำเนินการ:</b> คิวงานที่คุณกดจองไว้ และระบบกำลังรอให้ Admin ตรวจสอบเอกสาร</li>
                                    <li><b>✅ อนุมัติแล้ว:</b> คิวงานที่ Admin ตรวจสอบเอกสารครบถ้วนแล้ว (คิวสมบูรณ์)</li>
                                    <li><b>แก้ไข/ยกเลิก:</b> กดปุ่มจุดสามจุด (⋮) มุมขวาบนของการ์ด เพื่อแก้ไขหรือยกเลิกคิวงาน</li>
                                </ul>
                            )}
                        </div>
                    )}
                    ...
                    {/* ปุ่มเพิ่มวันลา (แสดงเฉพาะแท็บวันลาของ Inspector) */}
                                ...
                                <>
                                    {filteredTasks.slice(0, myBookingsLimit).map((h, i) => (
                                        <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 relative">
                                            {/* ปุ่ม Action (จุดสามจุด) สำหรับแก้ไข/ยกเลิก */}
                                            {(!isInspectorRole && myBookingsTab === 'pending') || (isInspectorRole && myBookingsTab === 'leave') ? (
                                                <div className="absolute top-3 right-3 z-10">
                                                    ...
                                                    {actionMenuId === h.id && (
                                                        <div className="absolute right-0 mt-1 w-32 bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden text-xs z-20">
                                                            {myBookingsTab === 'leave' ? (
                                                                <>
            ...
            {currentView === 'dashboard' && (
                <div className="page-view relative pb-20 animate-pop">
                    ...
                    {!isAdmin && !hasLoadedAdmin && user?.role === 'admin' ? (
                        ...
                        <div className="space-y-4 pb-10">
                            ...
                            {(() => {
                                ...
                                const siteCounts = allTasks.reduce((acc, task) => {
                                    if(task.site_name) {
                                        const cleanName = task.site_name.replace(/^[0-9:-]+\s/, ''); // ตัดเวลาด้านหน้าออกถ้ามี
                ...
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                                ...
                                {(modal.data?.role === 'inspector' || modal.isNew) && (
                                    <div className="bg-amber-50 p-3 rounded-xl border border-amber-200 mb-2 shadow-sm">
                                        ...
                                        <select name="inspector_mapped_name" defaultValue={modal.data.inspector_mapped_name || ''} className="w-full text-sm p-2.5 rounded-lg border border-amber-300 outline-none focus:border-amber-500 bg-white font-bold text-slate-700">
                                            ...
                                            {modal.data.inspector_mapped_name && !(db.inspectors || []).some(i => i.name === modal.data.inspector_mapped_name) && (
                    ...
                    {modal?.type === 'booking' && (
                        <div className="modal-card w-full max-w-[450px] animate-pop flex flex-col max-h-[90vh] bg-white rounded-3xl overflow-hidden shadow-2xl relative">
                            <button onClick={() => setModal(null)} className="absolute top-4 right-4 bg-slate-200 text-slate-500 p-2 rounded-full z-50 hover:bg-slate-300"><Icons.X /></button>
                            <div className="p-4 border-b bg-slate-50 flex justify-between items-center pr-14 flex-shrink-0">
                                <h3 className="text-xl font-bold text-slate-800">{modal.data.id ? '✏️ แก้ไขคิวงาน' : '📝 จองคิวงานใหม่'}</h3>
                            </div>
                                ...
                                <form onSubmit={handleBookingSubmit} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select value={productLineSelection} onChange={(e) => setProductLineSelection(e.target.value)} required className={`w-full text-sm p-2.5 rounded-lg border outline-none font-bold text-white shadow-sm ${PRODUCT_COLORS[productLineSelection] || 'bg-slate-500'}`}>
                                                <option value="" className="bg-white text-slate-700">-- เลือก Product --</option>
                                                {Object.keys(PRODUCT_COLORS).map(pl => <option key={pl} value={pl} className="bg-white text-slate-700">{pl}</option>)}
                                            </select>
                                            {productLineSelection === 'อื่นๆโปรดระบุ' && <input type="text" name="custom_product_line" required placeholder="ระบุ Product Line..." className="w-full text-sm p-2.5 mt-2 rounded-lg border bg-amber-50" />}
                                        </div>
                                        <div>
                                            <label className="text-xs font-bold text-slate-700 mb-1 block">ประเภทงาน <span className="text-red-500">*</span></label>
                                            <select value={jobTypeSelection} onChange={(e) => setJobTypeSelection(e.target.value)} required className="w-full text-sm p-2.5 rounded-lg border outline-none font-bold bg-slate-50 text-slate-800">
                                                <option value="">-- เลือกประเภทงาน --</option><option value="New">New</option><option value="MOD">MOD</option><option value="Re-ins temporary power supply">Re-ins temporary power supply</option><option value="Re-ins builder lift">Re-ins builder lift</option>
                                            </select>
                                        </div>
                                    </div>

                                    <div>
                                        <label className="text-xs font-bold text-slate-700 mb-1 block">พื้นที่รับผิดชอบ <span className="text-red-500">*</span></label>
                                        <select value={areaSelection} onChange={(e) => setAreaSelection(e.target.value)} required className="w-full text-sm p-2.5 rounded-lg border outline-none font-bold bg-slate-50">
                                            <option value="">-- เลือกพื้นที่ --</option><option value="กรุงเทพและปริมณฑล">กทม. และปริมณฑล</option><option value="เชียงใหม่">เชียงใหม่</option><option value="ภูเก็ต">ภูเก็ต</option><option value="other">อื่นๆ (ระบุเอง)</option>
                                        </select>
                                        {areaSelection === 'other' && <input type="text" name="custom_area" required placeholder="ระบุพื้นที่..." className="w-full text-sm p-2.5 mt-2 rounded-lg border bg-amber-50" />}
                                    </div>

                                    <div className="grid grid-cols-2 gap-3">
                                        <div><label className="text-xs font-bold text-slate-700 mb-1 block">Eq No. <span className="text-red-500">*</span></label><input type="text" name="equipment_no" required defaultValue={modal.data.equipment_no} placeholder="ระบุเลข Eq..." className="w-full text-sm p-2.5 rounded-lg border font-bold" /></div>
                                        <div><label className="text-xs font-bold text-slate-700 mb-1 block">Unit No. <span className="text-red-500">*</span></label><input type="text" name="unit_no" required defaultValue={modal.data.unit_no} placeholder="ตัวอย่าง: L1" className="w-full text-sm p-2.5 rounded-lg border font-bold" /></div>
                                    </div>

                                    <div>
                                        <label className="text-xs font-bold text-slate-700 mb-1 block">ชื่อโครงการ (Site Name) <span className="text-red-500">*</span></label>
                                        <input type="text" name="site_name" required defaultValue={modal.data.site_name?.replace(/^[0-9:-]+\s/, '')} placeholder="ระบุชื่อโครงการ..." className="w-full text-sm p-2.5 rounded-lg border font-bold" />
                                    </div>

                                    {isAdmin && (
                                        <div className="bg-red-50 p-4 rounded-xl border border-red-200">
                                            <label className="flex items-center gap-2 text-xs font-bold text-red-700 mb-3">
                                                <input type="checkbox" name="isAdminOverride" value="true" className="w-4 h-4 accent-red-600" defaultChecked={modal.data.id ? true : false} />
                                                🛠️ โหมด Admin (จองคิวข้ามวัน/เปลี่ยนคนตรวจ)
                                            </label>
                                            <div className="grid grid-cols-2 gap-2">
                                                <div><label className="text-[10px] font-bold text-slate-500 mb-1 block">ระบุวันที่</label><input type="date" name="admin_date_target" defaultValue={modal.data.date ? formatSafeDate(modal.data.date) : ''} className="w-full p-2 border rounded-lg text-sm bg-white" /></div>
                                                <div>
                                                    <label className="text-[10px] font-bold text-slate-500 mb-1 block">ระบุผู้ตรวจ</label>
                                                    <select name="admin_inspector_target" defaultValue={modal.data.inspector_name} className="w-full p-2 border rounded-lg text-sm bg-white">
                                    ...
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
                                        <h4 className="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 mb-3 flex items-center gap-2"><Icons.Upload /> อัปโหลดเอกสาร (PDF หรือ รูปภาพ)</h4>
                                        {['layout', 'wiring', 'precheck'].map((doc) => (
                                            <div key={doc} className="flex items-center justify-between bg-white p-2.5 rounded-lg border border-slate-200">
                                                <div className="flex flex-col">
                                                    <span className="text-xs uppercase font-bold text-slate-700">{doc}</span>
                                                    {docUrls[doc] ? <span className="text-[10px] text-green-600 font-bold">✅ อัปโหลดแล้ว</span> : <span className="text-[10px] text-slate-400">ยังไม่มีไฟล์</span>}
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {uploadingDoc[doc] && <Icons.Loader />}
                                                    <label className="text-[10px] bg-blue-50 text-blue-600 px-3 py-1.5 rounded-md font-bold cursor-pointer border border-blue-200 hover:bg-blue-100 transition-colors">
                                                        เลือกไฟล์
                                        ...
                                        <p className="text-[9px] text-slate-500 mt-2">หมายเหตุ: ไฟล์จะถูกบีบอัดและอัปโหลดไปเก็บใน Google Drive ทันที</p>
                                    </div>

                                    {!modal.data.id && isAdmin && (
                                        <label className="flex items-center gap-2 text-xs font-bold text-slate-600 bg-slate-50 p-3 rounded-lg border mt-2">
                                            <input type="checkbox" name="keep_open" className="w-4 h-4 accent-blue-600" />
                                            📌 บันทึกเสร็จแล้วไม่ต้องปิดหน้าต่าง (จองหลายตัวติดกัน)
                                        </label>
                                    )}

                                    <button type="submit" disabled={loadingMsg} className="w-full py-4 rounded-xl font-bold text-white bg-red-600 shadow-lg hover:bg-red-700 active:scale-95 transition-all mt-6 flex justify-center items-center gap-2">
                                        {loadingMsg ? <Icons.Loader /> : <Icons.Check />} ยืนยันการจองคิว
                                    </button>
                                </form>
                            </div>
                        </div>
                    ...
                    {modal?.type === 'inspector_leave_form' && (
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                <button type="submit" disabled={loadingMsg} className="w-full py-3.5 rounded-xl font-bold text-sm bg-blue-600 text-white shadow-md active:scale-95 transition-all">
                            ...
                            <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-slate-50 space-y-4">
                                <div className="bg-white p-4 rounded-xl shadow-sm border">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        }} className="w-full py-3 bg-amber-500 text-white font-bold rounded-xl shadow-md flex justify-center items-center gap-2 active:scale-95"><Icons.Plus /> เพิ่มวันลา ({leaveDates.length} วัน)</button>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-100">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            setEventStartDate(''); setEventEndDate(''); setEventInspectors([]); setShowEventDropdown(false); document.getElementById('event_name_input').value = '';
                                            
                                        }} className="w-full py-3.5 bg-emerald-500 text-white font-bold rounded-xl shadow-md flex justify-center items-center gap-2 active:scale-95 mt-2">
                                ...
                                <div className="mt-6 pt-4 border-t border-slate-200">
                                    <div className="flex justify-between items-center mb-3">
                                        ...
                                        {selectedEventsToDelete.length > 0 && (
                                            <button onClick={() => handleBulkDelete('event', selectedEventsToDelete)} className="bg-red-50 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-bold active:scale-95 shadow-sm border border-red-200 hover:bg-red-100">
                                        ...
                                        {(db.bookings || []).filter(b => b.job_type === 'company_event' && b.status !== 'cancelled').sort((a,b) => new Date(b.date) - new Date(a.date)).map(ge => {
                                            ...
                                            return (
                                                <div key={ge.id} className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm hover:border-emerald-300 transition-all">
                                                    ...
                                                    <div className="w-2 h-full min-h-[40px] rounded-full" style={{backgroundColor: customColor}}></div>
                            ...
                            <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-slate-50 space-y-4">
                                <div className="bg-white p-4 rounded-xl shadow-sm border">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        }} className="w-full py-3 bg-red-600 text-white font-bold rounded-xl shadow-md flex justify-center items-center gap-2 active:scale-95"><Icons.Plus /> กำหนดวันหยุด</button>
                                    </div>
                                </div>
                                ...
                                <div className="mt-6 pt-4 border-t border-slate-200">
                                    <div className="flex justify-between items-center mb-3">
                                        ...
                                        {selectedHolidaysToDelete.length > 0 && (
                                            <button onClick={() => handleBulkDelete('holiday', selectedHolidaysToDelete)} className="bg-red-50 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-bold active:scale-95 shadow-sm border border-red-200 hover:bg-red-100">
            ...
            {alertMsg && (
                <div className="backdrop z-[500] p-4 flex items-center justify-center">
                    <div className="bg-white w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl animate-pop">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2">แจ้งเตือน</h3>
                        <p className="text-sm text-slate-600 mb-6 whitespace-pre-line">{alertMsg}</p>
                        <button onClick={() => setAlertMsg(null)} className="w-full py-3 bg-slate-800 text-white font-bold rounded-xl shadow-md active:scale-95">ตกลง</button>
                    </div>
                </div>
            )}
            ...
            {promptDialog && (
                <div className="backdrop z-[600] p-4 flex items-center justify-center">
                    <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-pop">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setPromptDialog(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl active:scale-95">ยกเลิก</button>
            ...
            {/* 📍 หน้าต่างประวัติและการแจ้งเตือน (Activity Logs / Notifications) */}
            {showActivityModal && (
                <div className="backdrop z-[200]">
                    <div className="modal-card p-6 h-[85vh] flex flex-col w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2 flex-shrink-0 border-b pb-2"><Icons.List /> ประวัติการทำงานในระบบ</h3>
                        
                        {!hasLoadedAdmin ? (
                             <div className="flex flex-col items-center justify-center p-10 text-slate-400 gap-4 flex-1"><Icons.Loader /> <span className="text-xs">กำลังดึงประวัติข้อมูล (เฉพาะ Admin)</span></div>
                        ) : (
                            <div className="overflow-y-auto flex-1 pr-2 space-y-3 custom-scrollbar">
                                {(() => {
                                    ...
                                    return filteredLogs.slice(0, logsLimit).map((log, i) => (
                                        <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-slate-300 transition-colors">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex items-center gap-2">
                                            ...
                                            <div className="mb-2">
                                                <span className={`px-2 py-1 rounded-md text-[9px] font-bold inline-block shadow-sm ${String(log.action).includes('CREATE') || String(log.action).includes('เพิ่ม') ? 'bg-green-100 text-green-700 border-green-200' : String(log.action).includes('UPDATE') || String(log.action).includes('แก้ไข') ? 'bg-blue-100 text-blue-700 border-blue-200' : String(log.action).includes('DELETE') || String(log.action).includes('ลบ') ? 'bg-red-100 text-red-700 border-red-200' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
                                                    {log.action}
                                                </span>
                                            </div>
