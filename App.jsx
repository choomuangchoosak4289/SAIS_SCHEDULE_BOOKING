...
const App = () => {
    ...
    const handleTabChange = (view) => {
        setCurrentView(view);
        // 📍 ถ้าเข้าหน้า Admin, Search, หรือ Dashboard ให้โหลดข้อมูล Admin ทันที
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 แถบเมนูด้านล่าง (Bottom Nav) เพิ่มเมนู Dashboard แล้ว */}
            <div className="bottom-nav">
                ...
                {!isAdmin && <div className="nav-item text-red-500 hover:text-red-600" onClick={handleLogout}><Icons.LogOut /> ออกระบบ</div>}
            </div>
                ...
                <div className="page-view relative pb-20 animate-pop">
                    ...
                    {!hasLoadedAdmin ? (
                        <div className="text-center text-slate-400 p-10 flex flex-col items-center justify-center gap-4">
                            <Icons.Loader /> กำลังประมวลผลข้อมูล AI...
                        </div>
                    ) : (
                        <div className="space-y-4 pb-10">
                            {(() => {
                                ...
                                const allTasks = (adminDb.all_bookings || []).filter(b => 
                                    String(b.status) !== 'cancelled' &&
                                    !['leave', 'company_event', 'public_holiday'].includes(String(b.job_type).toLowerCase()) &&
                                    !String(b.equipment_no).startsWith('LEAVE_') && !String(b.equipment_no).startsWith('EVENT_') && !String(b.equipment_no).startsWith('HLD_')
                                );
                                ...
                                // Group By Area
                                const areaCounts = allTasks.reduce((acc, task) => {
                                    const area = task.area || 'ไม่ระบุ';
                                    ...
                                    <>
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-4 rounded-2xl shadow-sm text-white">
                                                <div className="text-[10px] font-bold text-blue-100 mb-1">คิวงานตรวจทั้งหมด</div>
                                        ...
                                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                                            <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2"><Icons.Award /> ผู้ตรวจที่มีงานเยอะที่สุด (Top 5)</h3>
                                            <div className="space-y-2">
                                                {topInspectors.map(([name, count], idx) => (
                                                    <div key={name} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${idx === 0 ? 'bg-amber-100 text-amber-600' : idx === 1 ? 'bg-slate-200 text-slate-600' : idx === 2 ? 'bg-orange-100 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>
                                                                {idx + 1}
                                                            </div>
                                                            <span className="font-bold text-slate-700 text-xs">{name}</span>
                                                        </div>
                                                        <div className="font-black text-blue-600 text-sm">{count} <span className="text-[10px] font-normal text-slate-400">คิว</span></div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        {/* Area Distribution */}
                                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                                            <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2"><Icons.MapPin /> สัดส่วนพื้นที่ปฏิบัติงาน</h3>
                                            <div className="grid grid-cols-2 gap-3">
                                                {Object.entries(areaCounts).sort((a,b)=>b[1]-a[1]).map(([area, count]) => (
                                                    <div key={area} className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-center">
                                                        <div className="text-lg font-black text-indigo-700">{count}</div>
                                                        <div className="text-[10px] font-bold text-indigo-900 mt-1 truncate">{area}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
