...
// 📍 นำเอา Settings มาใช้สำหรับการ์ด และกำหนด Fallback กันค่าสูญหาย
...
// 📍 แก้ปัญหาตารางพัง: ปล่อยให้ React ควบคุม Inline Styles เต็มที่ (ป้องกัน Grid ซ้อนกัน)
const CalendarGrid = React.memo(({ daysInView, db, isAdmin, user, setModal, setAlertMsg, handleDrop, handleDragOver, handleDragLeave, handleDragStart, handleDragEnd, setConfirmDialog, apiAction, setQuickAddType, filteredBookings, tableFontScale, columnZoom, specialFontScale, isExporting }) => {
    ...
    // ดึงค่าการตั้งค่าจาก Admin ถ้ายกเลิกการตั้งค่าหรือไม่มีค่า ให้ใช้ค่าเริ่มต้น
    ...
    // นี่คือเส้นเลือดหลักของตาราง - ต้องเป็น Inline เท่านั้นเพื่อป้องกัน CSS Overrides พัง
...
const App = () => {
    ...
    // 📍 State ควบคุมคู่มือการใช้งาน
    const [showBookingHelp, setShowBookingHelp] = useState(false);
    const [showLoginHelp, setShowLoginHelp] = useState(false);
    const [showAdminHelp, setShowAdminHelp] = useState(false);
    const [showRoleHelp, setShowRoleHelp] = useState(false);

    // 📍 ระบบประหยัดพลังงาน Enterprise: ตรวจสอบสถานะการพับจอ/ซ่อนแท็บ เพื่อหยุด Polling ชั่วคราวเมื่อไม่มีการใช้งาน
    useEffect(() => {
    ...
    // 📍 Dynamic CSS Variables Injector สำหรับปรับแต่งเว็บไซต์แบบ Real-time โดยไม่ทำลายโครงสร้าง Grid
    const DynamicStyles = () => {
        const s = db.settings || {};
        return (
            <style>{`
    ...
    // 📍 ระบบแจ้งเตือนรายบุคคล: เช็คว่าใน isRead มี Username ของเราหรือยัง
    const unreadNotifs = useMemo(() => {
        return (db.notifications || []).filter(n => {
            const isTargeted = n.target === user?.username || (isAdmin && n.target === 'ALL_ADMIN');
            const hasRead = String(n.isRead || '').split(',').includes(user?.username);
    ...
    // 📍 Smart Polling: หยุดดึงข้อมูลอัตโนมัติหากผู้ใช้พับจอหรือปิดหน้าจอมือถือทิ้งไว้ ประหยัดโควต้า 90%
    useEffect(() => {
        const load = async () => {
        ...
        const scheduleNextFetch = () => {
            ...
            timeoutId = setTimeout(() => {
                ...
                const isIdle = idleTime > 15 * 60 * 1000; // หากไม่มีการใช้งานเกิน 15 นาทีถือว่า Idle
                
                // ตรวจสอบว่าหน้าจอยังเปิดอยู่หรือไม่ (Visibility API)
    ...
    const handleDrop = async (e, targetDate, targetInspector) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                const ok = await apiAction({ action: 'update_booking', id: task.id, date: targetDate, inspector_name: finalInspector, user: user?.username || 'admin', reason: logDetail, job_type: task.job_type, equipment_no: task.equipment_no }, 'กำลังย้ายข้อมูล...');
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        const payload = { 
            ...
            equipment_no: modal.data.equipment_no
        };
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                setLoadingMsg('กำลังกู้คืนบัญชี...');
                                ...
                                setLoadingMsg(null);
                                if (result.status === 'ok') { 
                                    setAlertMsg(`✅ กู้คืนบัญชีสำเร็จ!\nUsername ของคุณคือ: ${result.username}\nกรุณาล็อกอินด้วยรหัสผ่านใหม่`);
                                    setIsForgotMode(false); 
                                } else { setAlertMsg(result.message || 'ไม่พบข้อมูลที่ตรงกันในระบบ'); }
                            } else if (isRegisterMode) {
                                if(fd.get('password') !== fd.get('confirm_password')) return setAlertMsg('รหัสผ่านยืนยันไม่ตรงกัน');
                                const payload = { action: 'register', username: fd.get('username'), password: fd.get('password'), full_name: fd.get('full_name'), phone: fd.get('phone') };
                                const ok = await apiAction(payload, 'กำลังสมัครสมาชิก...', true);
                                if (ok) { setSuccessModal('สมัครสมาชิกสำเร็จ กรุณารอ Admin อนุมัติ'); setIsRegisterMode(false); }
                            } else {
                            ...
                            {isForgotMode && (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-slate-100">
                                        <div><label className="text-[10px] font-bold text-slate-500">ตั้งรหัสผ่านใหม่</label><input name="password" type="password" required placeholder="New Password" className="bg-slate-50 w-full p-2 rounded-lg border text-sm font-bold" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500">ยืนยันรหัสผ่านใหม่</label><input name="confirm_password" type="password" required placeholder="Confirm Password" className="bg-slate-50 w-full p-2 rounded-lg border text-sm font-bold" /></div>
                                    </div>
                                </>
                            )}

                            <button disabled={loadingMsg} className="w-full py-3.5 rounded-xl text-white font-bold bg-red-600 mt-2 shadow-md text-sm transition-all active:scale-95">
                                {loadingMsg ? 'รอสักครู่...' : isRegisterMode ? 'ยืนยันการสมัครสมาชิก' : isForgotMode ? 'กู้คืนบัญชี / รีเซ็ตรหัสผ่าน' : 'เข้าสู่ระบบ (LOGIN)'}
                            </button>
        ...
        <div className="app-container">
            ...
            <header className="main-header">
                <div className="flex items-center gap-2">
                    <h1 className="text-xl font-bold tracking-wide">{db.settings?.appName || 'SAIS BOOKING'}</h1>
                </div>
                <div className="flex items-center gap-2 relative">
                    ...
                    <button className="btn-icon relative" onClick={() => { setShowActivityModal(true); if(!hasLoadedAdmin) fetchAdminData(0, 50, 'all'); }}>
                        ...
                        {unreadNotifs.length > 0 && (
                            <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-black w-[18px] h-[18px] flex items-center justify-center rounded-full border border-white animate-pulse shadow-sm">
            ...
            {currentView === 'my_bookings' && !isAdmin && (
                <div className="page-view relative pb-20">
                        ...
                        <div className="bg-slate-800 text-white p-4 rounded-xl mb-4 shadow-md text-xs leading-relaxed animate-pop relative">
                            ...
                            {user?.role === 'inspector' ? (
                                <ul className="list-disc pl-4 space-y-1.5 text-slate-200">
                                    ...
                                    <li><b>🗄️ ประวัติที่ผ่านมา:</b> คิวงานที่วันที่ผ่านพ้นไปแล้ว</li>
                                    <li><b>การดูข้อมูล:</b> กดที่รายการเพื่อดูรายละเอียดเบอร์ติดต่อ, ตำแหน่ง Google Map และสามารถเปิดดูรูปภาพ Site Conditions ที่หน้างานแนบมาได้</li>
                                </ul>
                            ) : (
                                <ul className="list-disc pl-4 space-y-1.5 text-slate-200">
                                    ...
                                    <li><b>แก้ไข/ยกเลิก:</b> กดปุ่มจุดสามจุด (⋮) มุมขวาบนของการ์ด เพื่อแก้ไขรายละเอียด หรือยกเลิกคิวงาน <span className="text-red-400">(สามารถทำได้เฉพาะคิวที่ยังไม่ถึงวันตรวจจริงเท่านั้น)</span></li>
                                </ul>
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex gap-2 mb-4 bg-slate-100 p-1 rounded-lg">
                        <button onClick={() => setMyBookingsTab('pending')} className={`flex-1 py-2 text-xs font-bold rounded-md ${myBookingsTab === 'pending' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500'}`}>
                            {user?.role === 'inspector' ? '⏳ คิวรอตรวจ' : '⏳ รอดำเนินการ'}
                        </button>
                        <button onClick={() => setMyBookingsTab('approved')} className={`flex-1 py-2 text-xs font-bold rounded-md ${myBookingsTab === 'approved' ? 'bg-white shadow-sm text-green-600' : 'text-slate-500'}`}>
                            {user?.role === 'inspector' ? '✅ เอกสารครบแล้ว' : '✅ อนุมัติแล้ว'}
                        </button>
                        <button onClick={() => setMyBookingsTab('completed')} className={`flex-1 py-2 text-xs font-bold rounded-md ${myBookingsTab === 'completed' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500'}`}>🗄️ ประวัติที่ผ่านมา</button>
                    </div>
                    ...
                    <div className="space-y-3 pb-10">
                        {(() => {
                            ...
                            const filteredTasks = (db.bookings || []).filter(b => {
                                ...
                                if (isInspectorRole) {
                                    const mappedName = user?.inspector_mapped_name || user?.full_name;
                                ...
                                <>
                                    {filteredTasks.slice(0, myBookingsLimit).map((h, i) => (
                                        <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 relative">
                                            {!isInspectorRole && myBookingsTab === 'pending' && (
            ...
            {/* 5. หน้า Admin Panel จัดการทุกอย่าง */}
                ...
                <div className="page-view relative pb-20">
                    ...
                    {showAdminHelp && (
                        <div className="bg-blue-50 p-5 rounded-2xl border border-blue-200 mb-4 shadow-sm text-sm animate-pop">
                            ...
                            <div className="space-y-4 text-blue-900 text-xs leading-relaxed">
                                <div><b>👥 จัดการผู้ใช้งาน:</b> อนุมัติสิทธิ์พนักงานใหม่ (เปลี่ยนจาก Pending เป็น Approved) และรีเซ็ตรหัสผ่าน <br/><span className="text-red-600">จุดสำคัญ:</span> สำหรับผู้ตรวจ (Inspector) แอดมินต้อง <b>"ผูกชื่อในตาราง (Mapping Name)"</b> ให้ตรงกับชื่อในปฏิทินเสมอ เพื่อให้งานเด้งไปที่หน้า "งานฉัน" ของผู้ตรวจท่านนั้น</div>
                                ...
                                <div><b>🎨 ตั้งค่าสีเว็บไซต์:</b> ปรับแต่งสีสัน, ความกว้างตาราง, และขนาดฟอนต์ได้ด้วยตนเองผ่านเมนู <b>"ตั้งค่าสีเว็บไซต์ / ตาราง"</b></div>
                            </div>
                        </div>
                    )}
                    ...
                    {adminTab === 'menu' && (
                        <div className="grid grid-cols-2 gap-4 animate-pop pb-10">
                            ...
                            {/* 📍 ปุ่มตั้งค่าเว็บไซต์ / ตาราง ตรงนี้ */}
                    ...
                    {/* 📍 Tab ใหม่สำหรับ Admin ใช้ปรับตั้งค่าหน้าตาเว็บไซต์ทั้งหมดแบบไร้โค้ด */}
                    {adminTab === 'web_settings' && (
                        <div className="space-y-4 animate-pop pb-10">
                            <h3 className="font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><Icons.Settings /> ปรับแต่งหน้าตาเว็บไซต์ (UI Customization)</h3>
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div><label className="text-xs font-bold text-slate-700 block mb-1">ชื่อแถบหัวเรื่อง (App Name)</label><input type="text" name="appName" defaultValue={db.settings?.appName || 'SAIS BOOKING'} className="w-full p-2 border rounded-lg text-sm font-bold bg-slate-50" /></div>
                                    <div className="grid grid-cols-3 gap-2">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">ความกว้างคอลัมน์ (px)</label><input type="number" name="gridColWidth" defaultValue={db.settings?.gridColWidth || 115} className="w-full p-2 border rounded-lg text-sm" /></div>
                                        ...
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">สีพื้นหลังหัวตาราง</label><input type="color" name="tableHeaderBg" defaultValue={db.settings?.tableHeaderBg || '#1e293b'} className="w-full h-10 border rounded cursor-pointer" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">สีตัวอักษรหัวตาราง</label><input type="color" name="tableHeaderText" defaultValue={db.settings?.tableHeaderText || '#ffffff'} className="w-full h-10 border rounded cursor-pointer" /></div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">ความโค้งมนของการ์ด (px)</label><input type="number" name="cardRadius" defaultValue={db.settings?.cardRadius || 6} className="w-full p-2 border rounded-lg text-sm" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">ระยะขอบใน (Padding px)</label><input type="number" name="cardPadding" defaultValue={db.settings?.cardPadding || 4} className="w-full p-2 border rounded-lg text-sm" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">ขนาดฟอนต์หัวข้อ (px)</label><input type="number" step="0.5" name="titleFontSize" defaultValue={db.settings?.titleFontSize || 11} className="w-full p-2 border rounded-lg text-sm text-blue-600 font-bold" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">ขนาดฟอนต์รายละเอียด (px)</label><input type="number" step="0.5" name="subFontSize" defaultValue={db.settings?.subFontSize || 10} className="w-full p-2 border rounded-lg text-sm text-blue-600 font-bold" /></div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-y-3 gap-x-2">
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1">งานปกติ (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="normalBg" defaultValue={db.settings?.normalBg || '#e2e8f0'} className="w-1/2 h-8" /><input type="color" name="normalText" defaultValue={db.settings?.normalText || '#1e293b'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1">งาน MOD (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="modBg" defaultValue={db.settings?.modBg || '#64748b'} className="w-1/2 h-8" /><input type="color" name="modText" defaultValue={db.settings?.modText || '#ffffff'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1">ต่างจังหวัด (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="upcBg" defaultValue={db.settings?.upcBg || '#f472b6'} className="w-1/2 h-8" /><input type="color" name="upcText" defaultValue={db.settings?.upcText || '#ffffff'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1">Re-ins/Temp (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="reinsBg" defaultValue={db.settings?.reinsBg || '#fef08a'} className="w-1/2 h-8" /><input type="color" name="reinsText" defaultValue={db.settings?.reinsText || '#854d0e'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1 text-red-600">วันหยุด (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="holidayBg" defaultValue={db.settings?.holidayBg || '#D0021B'} className="w-1/2 h-8" /><input type="color" name="holidayText" defaultValue={db.settings?.holidayText || '#ffffff'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded"><label className="text-[10px] font-bold block mb-1 text-amber-600">วันลา (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="leaveBg" defaultValue={db.settings?.leaveBg || '#eab308'} className="w-1/2 h-8" /><input type="color" name="leaveText" defaultValue={db.settings?.leaveText || '#ffffff'} className="w-1/2 h-8" /></div>
                                        </div>
                                        <div className="border p-2 rounded col-span-2"><label className="text-[10px] font-bold block mb-1 text-emerald-600">กิจกรรมบริษัท (พื้น / อักษร)</label>
                                            <div className="flex gap-1"><input type="color" name="eventBg" defaultValue={db.settings?.eventBg || '#22c55e'} className="w-1/2 h-8" /><input type="color" name="eventText" defaultValue={db.settings?.eventText || '#ffffff'} className="w-1/2 h-8" /></div>
                                        </div>
                                    </div>
                                </div>
                                
                                <button type="submit" className="w-full py-4 mt-4 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition-all text-sm">
                            ...
                            (adminDb.users || []).map((u, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2 hover:border-blue-300 transition-all">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        <button onClick={() => setModal({ type: 'edit_user', data: u })} className="text-[11px] bg-blue-50 text-blue-600 py-2 rounded-lg font-bold border border-blue-200 hover:bg-blue-100 flex items-center justify-center gap-1 col-span-2">
                                            <Icons.Edit /> แก้ไข/เปลี่ยนสิทธิ์
                                        </button>
                                        <button onClick={() => apiAction({action: 'update_user_status', target_user: u.username, new_status: u.status === 'approved' ? 'pending' : 'approved', admin_user: user?.username}, 'กำลังอัปเดต...')} className={`text-[11px] py-2 rounded-lg font-bold border col-span-1 ${u.status === 'approved' ? 'bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100'}`}>
                                        ...
                                        }} className="text-[11px] bg-red-50 text-red-600 py-2 rounded-lg font-bold border border-red-200 hover:bg-red-100 col-span-1">ลบ</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                    ...
                    {modal?.type === 'inspector_form' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                <input type="text" name="ins_name" defaultValue={modal.data?.name || ''} required placeholder="ระบุชื่อ-นามสกุล หรือ รหัส..." className="w-full text-sm p-3.5 rounded-xl border border-slate-300 outline-none mb-5 font-bold bg-slate-50" />
                                <div className="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 mb-6">
                                    <label className="text-xs font-bold text-indigo-900 block mb-2">กำหนดสิทธิ์ (Certificate)</label>
                                    <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                                        {Object.keys(PRODUCT_COLORS).filter(k => k !== 'อื่นๆโปรดระบุ').map(pl => (
                                            <label key={pl} className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer p-2 bg-white border border-slate-200 rounded-xl shadow-sm">
                                                <input type="checkbox" name="certs" value={pl} defaultChecked={modal.data?.product_lines?.includes(pl)} className="accent-indigo-600 w-4 h-4" />
                                ...
                                <div className="flex gap-3">
                                    <button type="button" onClick={() => setModal(null)} className="w-1/3 py-3.5 bg-slate-100 text-slate-600 font-bold rounded-xl">ยกเลิก</button>
                                    <button type="submit" disabled={loadingMsg} className="w-2/3 py-3.5 bg-indigo-600 text-white font-bold rounded-xl shadow-md flex items-center justify-center gap-2">
                                        {loadingMsg ? <Icons.Loader /> : <Icons.Check />} บันทึก
                                    </button>
                                </div>
                            </form>
                        </div>
                    ...
                    {modal?.type === 'admin_cell_action' && (
                        <div className="modal-card p-6 text-center animate-pop w-full max-w-sm bg-white rounded-3xl shadow-2xl relative">
                            ...
                            <div className="space-y-3">
                                <button onClick={() => { setAreaSelection(''); setJobTypeSelection(''); setProductLineSelection(''); setModal({ type: 'booking', data: modal.data }); }} className="w-full py-3.5 bg-blue-600 text-white font-bold rounded-xl shadow-md flex items-center justify-center gap-2"><Icons.Plus /> จองคิวตรวจ SAIS</button>
                                <button onClick={() => { setEventStartDate(modal.data.date); setEventEndDate(modal.data.date); setEventInspectors([]); setModal({ type: 'manage_events' }); }} className="w-full py-3.5 bg-emerald-500 text-white font-bold rounded-xl shadow-md flex items-center justify-center gap-2"><Icons.Star /> เพิ่มกิจกรรม</button>
                                <button onClick={() => { setLeaveStartDate(modal.data.date); setLeaveEndDate(modal.data.date); setLeaveInspectors([modal.data.inspector_name]); setModal({ type: 'manage_leaves' }); }} className="w-full py-3.5 bg-amber-500 text-white font-bold rounded-xl shadow-md flex items-center justify-center gap-2"><Icons.User /> จองวันลา</button>
                                <button onClick={() => { setHolidayStartDate(modal.data.date); setHolidayEndDate(modal.data.date); setModal({ type: 'manage_holidays' }); }} className="w-full py-3.5 bg-red-600 text-white font-bold rounded-xl shadow-md flex items-center justify-center gap-2"><Icons.CalendarX /> เพิ่มวันหยุด</button>
                            </div>
                        </div>
                    )}
                                ...
                                {!String(modal.data.job_type).includes('leave') && !String(modal.data.job_type).includes('event') && !String(modal.data.job_type).includes('holiday') && (
                                    <>
                                                ...
                                                {['layout', 'wiring', 'precheck'].map(docKey => {
                                                    ...
                                                    return (
                                                        <div key={docKey} className={`flex flex-col items-center justify-center p-2 rounded-xl border relative ${isSent ? 'bg-green-50 border-green-200 text-green-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                                                            ...
                                                            <div className="text-[11px] font-bold mb-1">{isSent ? '✅ ตรวจสอบแล้ว' : '⏳ รอตรวจสอบ'}</div>
                                                            {fileUrl && (
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <div className="mt-4 pt-4 border-t border-slate-100">
                                                    <h4 className="text-xs font-bold text-slate-800 mb-2 flex items-center gap-1"><Icons.Image /> รูปภาพ Site Conditions</h4>
                                        ...
                                        {modal.data.map_link && (
                                            <div className="mt-4 pt-4 border-t border-slate-100">
                                                ...
                                                <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50 h-44 cursor-pointer shadow-sm group" onClick={() => handleMapClick(modal.data.map_link)}>
                                                    <div className="absolute inset-0 z-10 bg-black/5 hover:bg-black/10 transition-colors"></div>
                                        ...
                                        return (
                                            <form onSubmit={handleBookingSubmit} className="space-y-4">
                                                ...
                                                <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 space-y-3 mt-4">
                                                    <h4 className="text-sm font-bold text-blue-800 border-b border-blue-200 pb-2 flex items-center gap-2"><Icons.Image /> อัปโหลดรูป Site Conditions (1-5 รูป)</h4>
                                                    {[
                                                        ...
                                                        return (
                                                            <div key={cond.id} className="flex items-center justify-between gap-2 bg-white p-2 rounded-lg border border-blue-100 shadow-sm">
                                                                ...
                                                                <div className="flex flex-col gap-1 items-end">
                                                                    ...
                                                                    <label className="bg-blue-600 text-white text-[10px] px-3 py-1.5 rounded cursor-pointer flex-shrink-0">
                                                ...
                                                <button disabled={loadingMsg} className="w-full py-3.5 rounded-xl font-bold text-white bg-red-600 shadow-md mt-6 active:scale-95 text-sm transition-transform">
                    ...
                    {/* Modal จัดการวันลา วันหยุด กิจกรรม */}
            ...
            {/* 📍 หน้าต่างแจ้งเตือน (อัปเดตให้แสดงจุดแดงที่รายการที่ยังไม่ได้อ่านแบบรายบุคคล .split(',') ) */}
            {showActivityModal && (
                <div className="backdrop z-[200]">
                    <div className="modal-card p-6 h-[85vh] flex flex-col relative">
                        <button onClick={() => setShowActivityModal(false)} className="btn-close-modern"><Icons.X /></button>
                        <h3 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2 flex-shrink-0"><Icons.Bell /> การแจ้งเตือน & ประวัติ</h3>
                        
                        {!hasLoadedAdmin ? (
                             <div className="flex flex-col items-center justify-center p-10 text-slate-400 gap-4 flex-1"><Icons.Loader /> กำลังดึงข้อมูล...</div>
                        ) : (
                            <div className="overflow-y-auto flex-1 pr-2 space-y-3 custom-scrollbar">
                                {(() => {
                                    // รวม Notifs และ Logs เข้าด้วยกันเพื่อเรียงตามเวลา
                                    const allEvents = [
                                        ...(db.notifications || []).filter(n => n.target === user?.username || (isAdmin && n.target === 'ALL_ADMIN')).map(n => ({...n, type: 'notif'})),
                                        ...(adminDb.logs || []).map(l => ({...l, type: 'log'}))
                                    ].sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));

                                    const filteredEvents = allEvents.filter(ev => {
                                        if (!logSearchQuery) return true;
                                        const q = logSearchQuery.toLowerCase();
                                        return String(ev.user || '').toLowerCase().includes(q) || String(ev.message || ev.action || '').toLowerCase().includes(q) || String(ev.details || '').toLowerCase().includes(q);
                                    });

                                    if (filteredEvents.length === 0) return <div className="text-center text-slate-400 text-sm py-10">ไม่พบประวัติ</div>;

                                    return filteredEvents.slice(0, logsLimit).map((ev, i) => {
                                        if (ev.type === 'notif') {
                                            // 📍 ตรวจสอบการอ่านแบบรายบุคคล ป้องกันบัคชื่อคล้ายกัน
                                            const readers = String(ev.isRead || '').split(',');
                                            const isUnread = !readers.includes(user?.username);

                                            return (
                                                <div key={`notif_${i}`} onClick={() => { 
                                                    // 📍 ส่ง user เข้าไปเพื่อให้ Backend บันทึกว่าใครเป็นคนกดอ่าน
                                                    if(isUnread) apiAction({ action: 'read_notification', id: ev.id, user: user?.username }, null, true); 
                                                }}
                                                     className={`p-4 rounded-xl border shadow-sm cursor-pointer transition-colors ${isUnread ? 'bg-blue-50 border-blue-300' : 'bg-white border-slate-200'}`}>
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="font-bold text-sm text-slate-800 flex items-center gap-2">
                                                            {isUnread && <span className="w-2.5 h-2.5 rounded-full bg-blue-500 animate-pulse"></span>}
                                                            {ev.title || 'แจ้งเตือน'}
                                                        </div>
                                                        <div className="text-[9px] text-slate-400 bg-slate-50 px-2 py-1 rounded-md border">
                                                            {ev.timestamp ? new Date(ev.timestamp).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                                                        </div>
                                                    </div>
                                                    <div className="text-xs text-slate-600 leading-relaxed">{ev.message}</div>
                                                </div>
                                            );
                                        } else {
                                            // Render System Log
                                            return (
                                                <div key={`log_${i}`} className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm opacity-80">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex items-center gap-2">
                                                            <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-slate-500"><Icons.User /></div>
                                                            <div className="text-[10px] font-bold text-slate-700">{ev.user}</div>
                                                        </div>
                                                    ...
                                                    <div className="mb-2">
                                                        <span className={`px-2 py-1 rounded-md text-[9px] font-bold inline-block ${String(ev.action).includes('CREATE') || String(ev.action).includes('เพิ่ม') ? 'bg-green-100 text-green-700' : String(ev.action).includes('UPDATE') || String(ev.action).includes('แก้ไข') ? 'bg-blue-100 text-blue-700' : String(ev.action).includes('DELETE') || String(ev.action).includes('ลบ') ? 'bg-red-100 text-red-700' : 'bg-slate-200 text-slate-700'}`}>{ev.action}</span>
                                                    </div>
                                                    <div className="text-[11px] font-mono text-slate-600 leading-relaxed whitespace-pre-wrap">{ev.details}</div>
                                                </div>
                                            );
                                        }
                                    });
                                ...
                                {logsLimit < 200 && (
                                    <button onClick={() => setLogsLimit(prev => prev + 20)} className="w-full py-3 bg-slate-100 text-slate-700 font-bold rounded-xl mt-4 active:scale-95 transition-all">
