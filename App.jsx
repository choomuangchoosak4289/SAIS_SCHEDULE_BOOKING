...
// 📍 นำเอา Settings มาใช้สำหรับการ์ด
                            ...
                            return (
                                <div key={idx} 
                                    ...
                                    {cellTasks.map((task, tIdx) => {
                                        // 📍 อัปเดตให้ส่ง db.settings เข้าไปทำงานร่วมกัน
...
const App = () => {
        ...
        return (
            <style>{`
                :root {
                    ...
                    --grid-col-width: ${s.gridColWidth || '115'}px;
                ...
                .calendar-grid { grid-template-columns: 50px repeat(var(--col-count, 1), minmax(var(--grid-col-width), 1fr)) !important; }
                .task-content { 
    ...
    // 📍 กรองรายการแจ้งเตือนที่ "ยังไม่ได้อ่าน" โดยเช็คจากชื่อ Username เพื่อรองรับการอ่านแยกรายบุคคล
    const unreadNotifs = useMemo(() => {
        return (db.notifications || []).filter(n => {
            const isTargeted = n.target === user?.username || (isAdmin && n.target === 'ALL_ADMIN');
            // เช็คว่าใน isRead มี Username ของเราแล้วหรือยัง ถ้ายัง = Unread
            const hasRead = String(n.isRead || '').includes(user?.username);
    ...
    return (
        <div className="app-container">
            ...
            {/* ส่วนหัวของแอป (Header) ดึงชื่อ App จาก Settings */}
            <header className="main-header">
                ...
                <div className="flex items-center gap-2 relative">
                    ...
                    {/* 📍 ปรับการแสดงผลตัวเลขแจ้งเตือนที่รูปกระดิ่ง */}
            ...
            {/* 2. หน้าตรวจเอกสาร (Admin) - กรองเฉพาะงานตรวจจริง และนำปุ่มดาวน์โหลดออก */}
                                            ...
                                            {['layout', 'wiring', 'precheck'].map(docKey => {
                                                ...
                                                return (
                                                    <div key={docKey} className={`flex flex-col gap-1 items-center p-2 rounded-lg border ${isSent ? 'bg-green-50' : 'bg-slate-50'}`}>
                                                        ...
                                                        {/* นำปุ่มดาวน์โหลดออก เหลือเฉพาะปุ่มดูไฟล์ */}
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div><label className="text-[10px] font-bold text-slate-500 block mb-1">สีพื้นหลังหัวตาราง (ชื่อช่าง)</label><input type="color" name="tableHeaderBg" defaultValue={db.settings?.tableHeaderBg || '#1e293b'} className="w-full h-10 border rounded cursor-pointer" /></div>
            ...
            {/* 📍 หน้าต่างแจ้งเตือน (อัปเดตให้แสดงจุดแดงที่รายการที่ยังไม่ได้อ่านแบบรายบุคคล) */}
            {showActivityModal && (
                            ...
                            <div className="overflow-y-auto flex-1 pr-2 space-y-3 custom-scrollbar">
                                {(() => {
                                    ...
                                    return filteredEvents.slice(0, logsLimit).map((ev, i) => {
                                        if (ev.type === 'notif') {
                                            // 📍 ตรวจสอบการอ่านแบบรายบุคคล โดยเช็คว่ามี Username เราใน isRead หรือยัง
                                            const isUnread = !String(ev.isRead || '').includes(user?.username);
                                            return (
                                                <div key={`notif_${i}`} onClick={() => { 
                                                    // 📍 ส่ง user เข้าไปเพื่อให้ Backend ทราบว่าใครเป็นคนกดอ่าน
