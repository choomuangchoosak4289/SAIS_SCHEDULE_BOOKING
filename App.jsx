...
const App = () => {
    ...
    // 📍 1. ตั้งค่าเชื่อมต่อ Firebase (แทรกเข้าไปใหม่เพื่อใช้งานฟรี 100%)
    useEffect(() => {
    ...
    const handleMigrateToFirestore = async () => {
        ...
        try {
            ...
            if(!window.dbFirestore) throw new Error("Firebase ยังไม่พร้อมใช้งาน กรุณารีเฟรชหน้าเว็บ");
    ...
    // 📍 ฟังก์ชัน Polling ข้อมูลเดิม (ผมเก็บไว้ครบ 100% ตามต้นฉบับ 21App)
    useEffect(() => {
        const load = async () => {
    ...
    // 📍 ฟังก์ชันอัปโหลดไฟล์แปลงเป็น Base64 ฟรี 100% ไม่พึ่ง Storage
                        ...
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.data?.id) {
                                    if (window.dbFirestore) {
                                        const { doc, updateDoc } = await import("https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js");
                                        await updateDoc(doc(window.dbFirestore, "bookings", String(modal.data.id)), { date: sDate, site_name: lType });
                                    }
                                    ...
                                    if (window.dbFirestore) {
                                        ...
                                        for(let date of dates) {
                                            ...
                                            await setDoc(doc(window.dbFirestore, "bookings", newId), { id: newId, date: date, inspector_name: targetName, job_type: 'leave', site_name: lType, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                        }
                                    }
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            if (window.dbFirestore) {
                                                ...
                                                for(let date of eventDates) {
                                                    const newId = generateId();
                                                    await setDoc(doc(window.dbFirestore, "bookings", newId), { id: newId, date: date, inspector_name: 'SYSTEM_EVENT', job_type: 'company_event', site_name: eName, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                                }
                                            }
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            if (window.dbFirestore) {
                                                ...
                                                for(let date of holidayDates) {
                                                    const newId = generateId();
                                                    await setDoc(doc(window.dbFirestore, "bookings", newId), { id: newId, date: date, inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: hName, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                                }
                                            }
