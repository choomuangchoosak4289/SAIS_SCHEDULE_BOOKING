...
const App = () => {
    ...
    // ไว้สำหรับแนบกับ Wrapper เพื่อดักจับ Pull-to-refresh
    const scrollRef = useRef(null);

    // 📍 2. Dynamic CSS Variables Injector (การตั้งค่าสี UI แอดมิน)
    const DynamicStyles = () => {
        const s = db.settings || {};
        return (
            <style>{`
    ...
    // 📍 1. แก้ไขลอจิกระบบแจ้งเตือน: กรองข้อมูลเฉพาะที่ยังไม่ได้อ่าน โดยอิงตาม username (รองรับ id รายบุคคล)
    ...
    // 📍 3. Pull-to-refresh: วางระบบตรวจจับการลากหน้าจอลง (Touch Events) ที่เชื่อมกับ spinner
        ...
        <div className="app-container">
                ...
                <div className="flex items-center gap-2 relative">
                    ...
                    {showSettings && (
                        <div className="settings-menu animate-pop w-[260px] max-h-[80vh] overflow-y-auto custom-scrollbar">
                            <h4 className="text-sm font-bold border-b border-slate-200 pb-2 mb-3 text-slate-800 flex items-center gap-2"><Icons.Settings /> การตั้งค่ามุมมองของคุณ</h4>
                            <button onClick={handleExportJPG} className="w-full py-2.5 bg-blue-600 text-white text-xs font-bold rounded-lg mb-3 shadow-sm active:scale-95">
                                <span className="flex items-center justify-center gap-2"><Icons.Download /> บันทึกตารางหน้านี้ (JPG)</span>
                            </button>
            ...
            {currentView === 'calendar' && (
                <div className="grid-container relative overflow-hidden pb-16">
                    ...
                    {/* 📍 3. Pull-to-refresh Spinner Indicator */}
            ...
            {/* 5. หน้า Admin Panel จัดการทุกอย่าง (ตามเวอร์ชันเก่าที่ต้องการ + อัปเกรด) */}
            {currentView === 'admin' && isAdmin && (
                <div className="page-view relative pb-20">
                            ...
                            (adminDb.users || []).map((u, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2 hover:border-blue-300 transition-all">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        <button onClick={() => setModal({ type: 'edit_user', data: u })} className="text-[11px] bg-blue-50 text-blue-600 py-2 rounded-lg font-bold border border-blue-200 hover:bg-blue-100 flex items-center justify-center gap-1 col-span-2 shadow-sm">
                    ...
                    {/* เมนูผู้ตรวจ และ Certificate (คืนฟังก์ชันเวอร์ชันเก่ามาเต็มรูปแบบ) */}
                    {adminTab === 'inspectors' && (
                    ...
                    {/* เมนูจัดการวันกิจกรรม วันลา วันหยุด (คืนฟังก์ชันเวอร์ชันเก่ามาเต็มรูปแบบ) */}
            ...
            {modal && (
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                    ...
                    {/* Modal แก้ไขผู้ใช้งาน (Edit User) */}
                        ...
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <div className="flex items-center gap-3 mb-5 border-b border-slate-100 pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-500 mt-0.5">ระบุชื่อและเลือก Product Line ที่อนุญาตให้รับงาน</p>
                                </div>
                            </div>
                            <form onSubmit={async (e) => {
                                e.preventDefault();
                    ...
                    {/* 📍 Global Modal ใช้งานร่วมกับ Alert, Confirm, Notification */}
                    {/* ... Component ใช้งานร่วมกันที่อิมพอร์ตไว้แล้วในส่วนล่าง ... */}
            ...
            {promptDialog && (
                <div className="backdrop z-[600] p-4 flex items-center justify-center">
                    <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-pop">
                        ...
                        <input type="text" id="prompt_input" className="w-full p-3 border border-slate-300 rounded-xl mb-4 bg-slate-50 font-bold text-sm outline-none focus:border-blue-400" placeholder="พิมพ์เหตุผล..." autoFocus />
                        <div className="flex gap-3">
            ...
            {/* 📍 หน้าต่างแจ้งเตือนและประวัติ (แก้ปัญหาลอจิกอ่านแจ้งเตือน 100%) */}
            {showActivityModal && (
                <div className="backdrop z-[200]">
                    <div className="modal-card p-6 h-[85vh] flex flex-col relative w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-3 flex items-center gap-2 flex-shrink-0 mt-1"><Icons.Bell /> การแจ้งเตือน & ประวัติ</h3>
                                    ...
                                    return userNotifs.map((notif, i) => {
                                        ...
                                        return (
                                            <div key={`notif_${notif.id || i}`} onClick={() => { 
                                                // 📍 เมื่อคลิกที่รายการแจ้งเตือน ให้อัปเดตสถานะการอ่านทันที (Optimistic UI)
                                                if(isUnread) {
...
// 📍 Error Boundary (ดักจับข้อผิดพลาดระดับสูงสุด)
        ...
        if (this.state.hasError) {
            return (
                <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
                    ...
                    <button onClick={() => window.location.reload()} className="bg-red-600 text-white px-6 py-3 rounded-xl font-bold shadow-md active:scale-95 transition-transform">รีเฟรชหน้าเว็บ</button>
                </div>
            );
        }
...
// 📍 เริ่มต้น Render React Application
