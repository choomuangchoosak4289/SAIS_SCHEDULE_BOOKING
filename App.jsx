...
const App = () => {
    ...
    useEffect(() => {
        let navTimer;
        // 📍 ตัดการ Auto-hide ออก เพื่อล็อกเมนูให้อยู่ด้านล่างตลอดเวลาตามคำขอรูปที่ 5
        const handleUserActivity = () => { setIsNavVisible(true); clearTimeout(navTimer); navTimer = setTimeout(() => setIsNavVisible(true), 3500); };
    ...
    useEffect(() => {
        ...
        const scheduleNextFetch = () => {
            ...
            // 📍 แก้ไข: เพิ่มรอบการโหลดเป็น 5 นาที (300000 ms) ป้องกัน Quota เซิร์ฟเวอร์เต็ม
    ...
    const fetchCoreData = async (needPast = false, currentCache = null) => {
        ...
        try {
            ...
            const res = await fetch(SCRIPT_URL, { 
                method: 'POST', 
                // 📍 นำ API KEY ใส่เข้าไปในทุก Fetch
    ...
    const apiAction = async (payload, customLoadMsg = 'กำลังบันทึกข้อมูล...', disableAutoSync = false) => {
        ...
        try {
            // 📍 นำ API KEY ใส่เข้าไปในทุก Payload
    ...
    // 📍 ฟังก์ชันจัดการเอกสารสำหรับแอดมิน (เปลี่ยนสถานะเมื่อกด Checkbox)
    ...
    // 📍 ลบ handleDownloadFile ออกตามคำขอ เพื่อให้อ่านรูปและไฟล์อย่างเดียว
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 ให้ตั้งสถานะเป็น pending รอแอดมินมาติ๊ก
        if (!modal?.data?.id) {
    ...
    const handleLogout = () => {
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                } catch (error) {} finally { window.location.reload(); }
            }
        });
    };

    // 📍 100% Full Screen Login - กู้คืนเมนู สมัครสมาชิก และลืมรหัสผ่าน อย่างสมบูรณ์
    if (!user) {
        return (
                ...
                <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 relative overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div className="text-center mb-6 pt-4 flex-shrink-0">
                        ...
                        <h2 className="text-lg font-bold text-slate-800 tracking-tight">
                            {showLoginHelp ? 'คู่มือการใช้งาน' : (isForgotMode ? 'รีเซ็ตรหัสผ่าน' : (isRegisterMode ? 'สมัครสมาชิกใหม่' : 'เข้าสู่ระบบเพื่อใช้งาน'))}
                        </h2>
                    </div>
                    ...
                    {showLoginHelp ? (
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4 pb-4">
                            <div className="bg-slate-50 p-4 rounded-2xl border">
                                <p className="text-xs text-slate-600 leading-relaxed text-center font-bold">ระบบ SAIS Booking รองรับการสร้างคิวงานสำหรับช่างและการดูตารางสำหรับบุคคลทั่วไป การอนุมัติสิทธิ์ขึ้นอยู่กับ Admin โดยตรงครับ</p>
                            </div>
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                } else { setAlertMsg(result.message || 'ไม่พบข้อมูล หรือกรอกชื่อ/เบอร์โทรไม่ตรงกับตอนสมัคร'); }
                                ...
                                if (ok) { setSuccessModal('สมัครสำเร็จ รอแอดมินอนุมัติสิทธิ์ครับ'); setIsRegisterMode(false); }
                            } else {
                                ...
                                } catch (err) { setLoadingMsg(null); setAlertMsg('ระบบขัดข้อง โปรดลองใหม่'); }
                            }
                                ...
                                <div className="space-y-3 pb-2 border-b border-slate-100">
                                    <div className="bg-blue-50 p-3 rounded-xl border border-blue-200 mb-2">
                                        <p className="text-[10px] text-blue-800 leading-relaxed font-bold">
                                            กรอกชื่อและเบอร์โทรที่ใช้สมัครเพื่อตั้งรหัสผ่านใหม่ (ระบบจะแจ้ง Username ให้ทราบเมื่อสำเร็จ)
                                        </p>
                                    </div>
                            ...
                            {isRegisterMode && (
                                <div className="space-y-3 pb-2 border-b border-slate-100">
                                    <div><label className="text-[10px] font-bold text-slate-500">ชื่อ-นามสกุล</label><input name="full_name" required placeholder="ระบุชื่อ-นามสกุล" className="bg-slate-50 w-full p-2.5 rounded-lg border text-sm" /></div>
                                </div>
                            )}

                            {/* Username สำหรับ Login และ Register */}
                            {!isForgotMode && (
                            ...
                            {/* ยืนยันรหัสผ่าน สำหรับสมัครใหม่ หรือลืมรหัส */}
                            ...
                            <button disabled={loadingMsg} className="w-full py-3.5 rounded-xl text-white font-bold bg-red-600 mt-4 shadow-md text-sm">
                                {loadingMsg ? 'รอสักครู่...' : (isForgotMode ? 'รีเซ็ตรหัสผ่าน' : (isRegisterMode ? 'ส่งข้อมูลสมัครสมาชิก' : 'เข้าสู่ระบบ (LOGIN)'))}
                            </button>
                            
                            <div className="text-center mt-4">
                                {!isForgotMode && !isRegisterMode && (
                                    <>
                                        <button type="button" onClick={() => setIsForgotMode(true)} className="text-xs font-bold text-red-500 hover:underline mb-3 block w-full">ลืมรหัสผ่าน / ลืม Username ?</button>
                                        <button type="button" onClick={() => setIsRegisterMode(true)} className="text-xs font-bold text-blue-600 hover:underline mb-3 block w-full">คลิกที่นี่ หากยังไม่มีบัญชี (สมัครสมาชิก)</button>
                                    </>
                                )}
                                {(isRegisterMode || isForgotMode) && (
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 แถบเมนูด้านล่าง (ล็อกติดขอบล่าง 100% เอา Auto hide ออก) */}
            <div className="bottom-nav" style={{ 
                position: 'fixed', bottom: 0, left: 0, right: 0, 
                zIndex: 50, display: 'flex', background: 'white', 
                borderTop: '1px solid #e2e8f0', paddingBottom: 'env(safe-area-inset-bottom, 12px)',
            ...
            {currentView === 'calendar' && (
                <div className="grid-container relative overflow-hidden pb-[60px]">
            ...
            {currentView === 'documents' && isAdmin && (
                <div className="page-view relative pb-[80px]">
                    <div className="sticky top-0 bg-[#f1f5f9] z-10 pb-4 pt-2 border-b border-slate-200">
                        ...
                        <div className="text-xs text-slate-500 mt-1">เฉพาะคิวงานที่มีไฟล์แนบ แอดมินจึงจะสามารถติ๊กยืนยันได้</div>
                    </div>
                                ...
                                return (
                                    <div key={i} className={`bg-white p-4 rounded-xl shadow-sm border-2 transition-all ${all_ok ? 'border-green-200 bg-green-50/30' : 'border-slate-200'}`}>
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            {/* 📍 ปรับปรุงระบบ Checkbox แอดมิน: แสดงเฉพาะเมื่อมีไฟล์แนบเท่านั้น! ไม่มีปุ่มโหลด */}
                                            {['layout', 'wiring', 'precheck'].map(docKey => {
                                                const isSent = String(h[`${docKey}_doc`]) === 'true';
                                                const fileUrl = h[`${docKey}_img`];
                                                const hasFile = Boolean(fileUrl && fileUrl !== '');
                                                return (
                                                    <div key={docKey} className={`flex flex-col gap-1 items-center p-2 rounded-lg border ${isSent ? 'bg-green-50' : 'bg-slate-50'}`}>
                                                        ...
                                                        {hasFile ? (
                                                            <>
                                                                ...
                                                                <button onClick={() => setViewFileUrl(fileUrl)} className="w-full mt-1 text-[9px] bg-blue-600 text-white border border-blue-600 rounded py-1 font-bold shadow-sm active:scale-95">ดูไฟล์</button>
                                                            </>
                                                        ) : (
                                                            <div className="text-[9px] font-bold text-red-500 py-1 text-center w-full bg-red-50 rounded">❌ รออัพโหลดไฟล์</div>
                                                        )}
                                                    </div>
                                                );
                                            })}
                    ...
                    {adminTab === 'users' && (
                        <div className="space-y-3 mt-4 animate-pop pb-10">
                            <div className="flex justify-between items-center px-1 mb-2">
                                ...
                                {/* 📍 ปุ่มสร้าง Viewer สำหรับแอดมิน */}
                                <button onClick={() => setModal({ type: 'create_viewer' })} className="text-[10px] bg-indigo-600 text-white px-3 py-1.5 rounded-lg font-bold shadow-sm flex items-center gap-1 active:scale-95">
                                    <Icons.UserPlus /> สร้างบัญชี Viewer
                                </button>
                            </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold text-slate-800 flex items-center gap-2">
                                                    ...
                                                    {u.role === 'viewer' && <span className="bg-blue-100 text-blue-600 text-[9px] px-2 py-0.5 rounded-full font-black border border-blue-200 tracking-wide">VIEWER</span>}
                                                </div>
            ...
            {/* =========================================
                MODALS (หน้าต่างแจ้งเตือน/ฟอร์มต่างๆ)
            ========================================= */}
            {modal && (
                <div className="backdrop z-[100] p-4 flex items-center justify-center">

                    {/* 📍 Modal สำหรับแอดมินสร้างบัญชี Viewer */}
                    {modal?.type === 'create_viewer' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <h3 className="text-xl font-bold text-slate-900 mb-4 border-b pb-2 flex items-center gap-2"><Icons.UserPlus /> สร้างบัญชี Viewer (ดูตาราง)</h3>
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                setLoadingMsg('กำลังสร้างบัญชี...');
                                const regOk = await apiAction({ action: 'register', username: fd.get('username'), password: fd.get('password'), full_name: fd.get('full_name'), api_key: window?.SAIS_CONFIG?.API_KEY }, null, true);
                                if (regOk) {
                                    const updOk = await apiAction({
                                        action: 'update_user_details', target_user: fd.get('username'), admin_user: user?.username,
                                        full_name: fd.get('full_name'), role: 'viewer', status: 'approved', api_key: window?.SAIS_CONFIG?.API_KEY
                                    }, null);
                                    setLoadingMsg(null);
                                    if(updOk) { setSuccessModal('สร้างบัญชี Viewer สำเร็จ'); setModal(null); fetchAdminData(0, 50, 'users'); }
                                } else {
                                    setLoadingMsg(null);
                                }
                            }} className="space-y-3">
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Username (ใช้ล็อกอิน)</label>
                                    <input type="text" name="username" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none focus:border-blue-400" placeholder="ตั้ง Username..." />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">รหัสผ่าน</label>
                                    <input type="password" name="password" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none focus:border-blue-400" placeholder="ตั้งรหัสผ่าน..." />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">ยืนยันรหัสผ่าน</label>
                                    <input type="password" name="confirm_password" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none focus:border-blue-400" placeholder="ยืนยันรหัสผ่านอีกครั้ง..." />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">ชื่อที่แสดง (บริษัท / หน่วยงาน)</label>
                                    <input type="text" name="full_name" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none focus:border-blue-400" placeholder="ระบุชื่อหรือหน่วยงาน..." />
                                </div>
                                <div className="bg-blue-50 text-blue-700 text-[10px] p-3 rounded-lg font-bold leading-relaxed border border-blue-200 mt-2">
                                    บัญชีนี้จะสามารถ "เข้าดู" ตารางและไฟล์ได้อย่างเดียว ไม่สามารถจองคิว ลบ หรือแก้ไขข้อมูลใดๆ ได้
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-slate-100">
                                    <div className="bg-blue-50 p-2 rounded-lg border border-blue-100">
                                        ...
                                        <select name="role" defaultValue={modal.data.role || 'user'} className="w-full text-sm p-2.5 rounded-lg border border-blue-200 outline-none font-bold bg-white text-blue-900">
                                            ...
                                            {/* 📍 เพิ่ม Option สิทธิ์ Viewer ในหน้า Admin User Management */}
                                                ...
                                                {['layout', 'wiring', 'precheck'].map(docKey => {
                                                    ...
                                                    return (
                                                        <div key={docKey} className={`flex flex-col items-center justify-center p-2 rounded-xl border relative ${isSent ? 'bg-green-50 border-green-200 text-green-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                                                            ...
                                                            {/* 📍 ปรับเหลือแต่ปุ่ม เปิดดูไฟล์ (เอาปุ่มโหลดออก) */}
                                                            {fileUrl && (
            ...
            {/* 📍 Global Modal สำหรับคลิกเปิดดูไฟล์รูปภาพหรือเอกสาร (เหลือแค่แท็บเปิดไฟล์ ไม่มีปุ่มโหลด) */}
            {viewFileUrl && (
