...
const App = () => {
    ...
    // 📍 ดึงข้อมูล Realtime ด้วย Firestore (Compat) 100% (ลบ fetchCoreData แบบเดิมทิ้ง)
    useEffect(() => {
        let unsubs = [];
    ...
    const fetchCoreData = async () => {
        // ใช้ Firestore เป็นหลักแล้ว ฟังก์ชันนี้ไม่ต้องดึงจาก Sheets ป้องกันเว็บค้าง
        return true;
    };
    ...
    const fetchAdminData = async (offset = 0, limit = 50, fetchType = 'all') => {
        ...
        try {
            ...
            // Logs ยังดึงจาก Google Sheets ได้ เพราะไม่ได้กระทบกับหน้าจอปฏิทินหลัก
            if (SCRIPT_URL && (fetchType === 'all' || fetchType === 'logs')) {
                fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'sync_admin', api_key: window?.SAIS_CONFIG?.API_KEY, offset, limit, fetch_type: 'logs' }) })
                .then(res => res.json())
                .then(result => {
                    if (result.status === 'ok') setAdminDb(prev => ({ ...prev, logs: result.data.logs || [] }));
    ...
    // 📍 หน้าจอ Login (อัปเกรดให้เชื่อมตรงเข้า Firebase)
    if (!user) {
        return (
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                try {
                                    // สำหรับรีเซ็ตรหัสผ่าน ยังคงส่งไปที่ Apps Script ก่อนชั่วคราวเพื่อความซับซ้อนของการค้นหา
                                    ...
                                    if (result.status === 'ok') { 
                                        // อัปเดตใน Firebase ด้วย
                                        if (window.dbFirestore && result.username) {
                                            await window.dbFirestore.collection("users").doc(result.username).update({ password: fd.get('password') });
                                        }
                                ...
                                try {
                                    ...
                                    setLoadingMsg(null);
                                    setSuccessModal('สมัครสำเร็จ รอแอดมินอนุมัติ'); 
                                ...
                                // 📍 ระบบ Login ตรวจสอบผ่าน Firebase โดยตรง!
                                setLoadingMsg('กำลังตรวจสอบข้อมูล...');
                                try {
                                    if (!window.dbFirestore) throw new Error("Firebase ยังไม่พร้อมทำงาน กรุณารอสักครู่แล้วกดใหม่");
                                    const usernameInput = fd.get('username');
                                    ...
                                    setLoadingMsg(null); 
                                    setAlertMsg('การเชื่อมต่อฐานข้อมูลขัดข้อง: ' + err.message); 
                                }
                            }
                    ...
                    {!hasLoadedAdmin ? (
                        ...
                        <div className="space-y-4 pb-10">
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select className="w-full text-xs p-2.5 border rounded-lg bg-slate-50 font-bold outline-none text-slate-700" value={dashArea} onChange={e=>setDashArea(e.target.value)}>
                                        <option value="All">ทุกพื้นที่</option><option value="กรุงเทพและปริมณฑล">กทม. และปริมณฑล</option><option value="เชียงใหม่">เชียงใหม่</option><option value="ภูเก็ต">ภูเก็ต</option>
                                    </select>
                                </div>
                                <div>
                            ...
                            {(() => {
                                ...
                                const sortedInspectors = (db.inspectors || []).map(ins => ({ name: ins.name, count: inspectorCounts[ins.name] || 0 }));
                                                ...
                                                {sortedInspectors.map((ins, idx) => (
                                                    <div key={ins.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-8 h-8 rounded-full flex items-center justify-center font-black text-sm bg-indigo-100 text-indigo-600 border border-indigo-200">{idx + 1}</div>
                    ...
                    {adminTab === 'users' && (
                        <div className="space-y-3 mt-4 animate-pop pb-10">
                            <div className="flex justify-between items-center px-1 mb-3">
                                ...
                                <button onClick={() => setModal({ type: 'edit_user', isNew: true, data: { username: '', full_name: '', department: '', position: '', phone: '', role: 'user', status: 'approved' } })} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 shadow-sm active:scale-95"><Icons.Plus /> สร้างผู้ใช้งาน</button>
                            </div>
                            ...
                            {(adminDb.users || []).map((u, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2">
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-black">{u.username.charAt(0).toUpperCase()}</div>
                                            <div>
                                                <div className="font-bold text-slate-800 flex items-center gap-2">
                                                    {u.username}
                                                    {u.role === 'admin' && <span className="bg-red-100 text-red-600 text-[9px] px-2 py-0.5 rounded-full font-black">ADMIN</span>}
                                                    {u.role === 'inspector' && <span className="bg-indigo-100 text-indigo-600 text-[9px] px-2 py-0.5 rounded-full font-black">INSPECTOR</span>}
                                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t">
                                        <button onClick={() => setModal({ type: 'edit_user', isNew: false, data: u })} className="text-xs bg-blue-50 text-blue-600 py-2 rounded-lg font-bold border border-blue-200">แก้ไขสิทธิ์</button>
                                        <button onClick={() => {
                                            setConfirmDialog({
                                        ...
                                        }} className="text-xs bg-red-50 text-red-600 py-2 rounded-lg font-bold border border-red-200">ลบผู้ใช้</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                    {modal?.type === 'detail' && (
                        <div className="modal-card p-6 w-full max-w-md animate-pop bg-white rounded-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <button onClick={() => setModal(null)} className="w-full py-3 bg-slate-800 text-white font-bold rounded-xl shadow-md">ปิดหน้าต่าง</button>
                        </div>
                    )}
                </div>
            )}
...
// 📍 ระบบ ErrorBoundary ป้องกันหน้าเว็บขาว
