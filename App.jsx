...
const App = () => {
    ...
    // สถานะสำหรับการโหลด และ Firebase
    ...
    // ประกาศ State สำหรับ Drag & Drop ถังขยะ
    ...
    // 📍 2. ฟังก์ชันแก้ไขตัวแปรที่หายไป (ประกาศให้อยู่ใน Scope ก่อนการ Return ใดๆ)
    const isAdmin = useMemo(() => user?.role === 'admin', [user]);
    
    // 💡 ตรงนี้คือตัวแปรที่ทำหน้าจอขาวตอนแรก แก้ไขให้ครอบคลุม 100% แล้วครับ
    ...
    useEffect(() => {
        if (successModal) {
            const timer = setTimeout(() => { setSuccessModal(null); }, 3500); // 💡 ขยายเวลาเป็น 3.5 วินาทีให้อ่านทัน
            return () => clearTimeout(timer);
        }
    }, [successModal]);

    // 📍 6. จัดการ UI Settings
    ...
    // 📍 7. ระบบ Background Sync (Backup ไปยัง Google Sheets เสมอเพื่อความปลอดภัย)
    ...
    // 📍 8. ระบบลบข้อมูลแบบกลุ่ม (Bulk Delete)
    ...
    // 📍 9. ระบบอัปโหลดไฟล์ลง Storage หรือ Sheets
    const handleFileUpload = async (e, docType, isMultiple = false) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // สามารถใช้ Firebase Storage หรือ Apps Script ในการอัปโหลดไฟล์ (ตอนนี้ยิงผ่าน Apps Script เพื่อเก็บ URL ง่ายกว่า)
    ...
    // 📍 11. ฟังก์ชันจัดการการยกเลิกคิวงาน / วันลา
    ...
    // 📍 12. ฟังก์ชันแก้ไขข้อมูลพิเศษ (วันลา วันหยุด กิจกรรม)
    ...
    // 📍 13. ฟังก์ชัน Submit การจองคิวงาน และเพิ่มคิวพิเศษ (The Core Function)
                                            ...
                                            <form onSubmit={handleBookingSubmit} className="space-y-4">
                                                ...
                                                <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 space-y-3 mt-4">
                                                    <h4 className="text-sm font-bold text-blue-800 border-b border-blue-200 pb-2 flex items-center gap-2">
                                                        <Icons.Image /> อัปโหลดรูปสภาพหน้างาน <span className="text-[10px] text-red-500 font-normal bg-red-50 px-2 py-0.5 rounded-md ml-1 border border-red-100">(ผู้จองต้องแนบ 1-5 รูป)</span>
                                                    </h4>
                                                    {[
                ...
                <div className="backdrop z-[200] p-4 flex items-center justify-center">
                    <div className="modal-card p-6 w-full max-w-lg bg-white rounded-3xl shadow-2xl h-[85vh] flex flex-col animate-pop relative">
                        ...
                        <div className="flex gap-2 mb-4 bg-slate-100 p-1 rounded-xl flex-shrink-0">
                            ...
                            <button onClick={() => { setActivityTab('log'); if(!hasLoadedAdmin && isAdmin) fetchAdminData(0, 50, 'logs'); }} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activityTab === 'log' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500'}`}>
                                🗄️ ประวัติระบบ (Logs)
                            </button>
                        </div>

                        {!hasLoadedAdmin && activityTab === 'log' ? (
                                        ...
                                        return userNotifs.map((notif, i) => {
                                            ...
                                            return (
                                                <div key={`notif_${notif.id || i}`} onClick={() => { 
                                                    if(isUnread) {
                                                        const newReaders = [...readers, user?.username].filter(Boolean).join(',');
                                                        setDb(prev => ({
                                                            ...prev,
                                                            notifications: prev.notifications.map(n => n.id === notif.id ? { ...n, isRead: newReaders } : n)
                                                        }));
                                                        fetch(SCRIPT_URL, { 
                                                            method: 'POST', 
                                                            body: JSON.stringify({ action: 'read_notification', api_key: window?.SAIS_CONFIG?.API_KEY, id: notif.id, user: user?.username }) 
                                                        });
                                                    }
                                                }}
                                                     className={`p-4 rounded-xl border shadow-sm cursor-pointer transition-all ${isUnread ? 'bg-blue-50/80 border-blue-300 ring-1 ring-blue-300 hover:bg-blue-100' : 'bg-white border-slate-200 opacity-75 hover:opacity-100'}`}>
                                                    <div className="flex justify-between items-start mb-2">
        ...
        if (this.state.hasError) {
            return (
                <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
                    <div className="text-red-500 mb-4 bg-red-50 p-4 rounded-full shadow-sm">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
                        </svg>
                    </div>
                    <h2 className="text-xl font-bold text-slate-800 mb-2">ระบบขัดข้องชั่วคราว</h2>
                    <p className="text-sm text-slate-500 mb-6 bg-white p-3 rounded-lg max-w-md break-words border border-slate-200 shadow-sm">{this.state.errorMsg}</p>
