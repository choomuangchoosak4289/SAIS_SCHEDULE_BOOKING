...
const App = () => {
    ...
    // 📍 State สำหรับ Dashboard อัจฉริยะ
    ...
    // 📍 2. ฟังก์ชันย้ายฐานข้อมูลจาก Sheets ไป Firestore (ระบบ Batch อัปเดต ป้องกันเว็บค้าง)
    const handleMigrateToFirestore = async () => {
        ...
        try {
            ...
            for (let i = 0; i < bookings.length; i++) {
                ...
                // Commit ทุกๆ 400 รายการ ป้องกัน Firebase Limit
                if (batchCount >= 400) {
                    await batch.commit();
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            try {
                ...
                if (window.dbFirestore) {
                    await window.dbFirestore.collection("bookings").doc(docId).set({
    ...
    // 📍 หน้าจอ Login (ระบบเข้าสู่ระบบ กู้รหัส และสมัครสมาชิก โฉมใหม่ แบบ Hybrid 100%)
    if (!user) {
        return (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                try {
                                    ...
                                    // ถ้าใน Firebase ยังไม่มีข้อมูล ให้ค้นหาใน Google Sheets (Fallback)
                                    if (!isFound) {
                                        const res = await fetch(SCRIPT_URL, { 
                                        ...
                                        const result = await res.json();
                                        if (result.status === 'ok') { 
                                            // บันทึกลง Firebase ด้วยเพื่อให้ครั้งหน้าเร็วขึ้น
                            ...
                            // 📍 โหมดสมัครสมาชิกใหม่ (Hybrid)
                                ...
                                try {
                                    ...
                                    setLoadingMsg(null);
                                    setSuccessModal('สมัครสำเร็จ รอแอดมินอนุมัติสิทธิ์'); 
                                    setIsRegisterMode(false); 
                                } catch (err) {
                                    setLoadingMsg(null);
                                    setAlertMsg('สมัครไม่สำเร็จ: ' + err.message);
                                }
                            } 
                            // 📍 โหมดล็อกอินเข้าสู่ระบบ (Hybrid System: แก้ปัญหา Firebase ว่างเปล่าตอนเข้าใช้งานครั้งแรก)
                            else {
                                ...
                                try {
                                    ...
                                    // 2. ถ้าใน Firebase ไม่มีข้อมูล (เช่น ระบบยังไม่ได้กดย้ายฐานข้อมูล) ให้ค้นหาผ่าน Google Sheets
                                    ...
                                    if (userData) {
                                        if (userData.status === 'blocked') {
                                            ...
                                            delete userData.password; // ลบรหัสผ่านเพื่อความปลอดภัยก่อนเก็บลงเครื่อง
...
// 📍 ระบบดักจับข้อผิดพลาด (ป้องกันหน้าเว็บขาว)
