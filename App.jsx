                                ...
                                <div key={idx} 
                                    ...
                                    onClick={() => {
                                        ...
                                        if (isAdmin) {
                                            ...
                                            // 📍 แก้ปัญหาค้างตอนกดจองคิว โดยส่งเฉพาะข้อมูลที่จำเป็นและมั่นใจว่าไม่มี Infinite Render
                                            setQuickAddType('job');
                                            setAreaSelection(''); 
                                            setJobTypeSelection(''); 
                                            setProductLineSelection('');
                                            setModal({ type: 'booking', data: { date: d.full, inspector_name: ins.name, id: null } });
                                        }
                                    }}>
...
// ค้นหา ID อัตโนมัติถ้าไม่มี ID (ป้องกัน Error ไม่พบไอดีใน Firebase)
    ...
    const handleFileUpload = async (e, docType, isMultiple = false) => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                // การอัปโหลดรูปยังคงต้องยิงไป Apps Script เพื่อเก็บลง Google Drive ให้เรียบร้อย
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        } catch (e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาด'); setLoadingMsg(null); }
    };
...
// 📍 หน้าจอ Login (ระบบ Firebase 100% ถอด Google Sheets ออก)
    if (!user) {
        return (
    ...
    if (!SCRIPT_URL && !isFirebaseReady) return <div className="h-screen w-full flex items-center justify-center flex-col gap-4 p-8 text-center"><Icons.Loader /><h2 className="text-xl font-bold text-slate-800">กำลังเตรียมระบบฐานข้อมูล...</h2></div>;
...
{/* =========================================
            ...
            {/* 3. หน้าค้นหา (อัปเกรดค้นหา 5 หลัก) */}
            {currentView === 'search' && (
                <div className="page-view relative pb-20">
                    ...
                    <div className="space-y-3 pb-10">
                        {!hasLoadedAdmin ? (
                            ...
                            <div className="text-center text-amber-600 p-8 border-2 border-dashed border-amber-300 bg-amber-50 rounded-xl mt-4">
                                ...
                                กรุณาพิมพ์ตัวอักษรหรือตัวเลขอย่างน้อย <b className="text-amber-700">5 หลัก</b><br/>เพื่อเริ่มการค้นหา...
                            </div>
                        ) : (
                            (() => {
                        ...
                        <div className="space-y-4 animate-pop pb-10">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    // การอัปเดตการตั้งค่าจะยังคงยิง Background Sync เพื่อ Backup ตามเดิม
