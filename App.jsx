...
// 📍 ฟังก์ชันสร้าง ID ป้องกันการซ้ำซ้อน
                                    ...
                                    {d.isGlobalHoliday && d.globalHolidays.map((gh, ghi) => {
                                        ...
                                        return (
                                            <div key={'gh'+ghi} 
                                                ...
                                                className={isCard ? `task-content relative w-full flex items-center justify-center p-1 rounded-md mb-1 ${isAdmin ? 'cursor-grab active:cursor-grabbing hover:ring-2 hover:ring-black/20' : 'cursor-pointer'}` : `holiday-label-new flex-1 flex items-center justify-center text-center ${isAdmin ? 'cursor-grab active:cursor-grabbing hover:opacity-80' : 'cursor-pointer'}`} 
                                                style={{ 
...
const App = () => {
    ...
    // 🟢 แก้บัคที่ทำให้เกิด ReferenceError: setHasLoadedAdmin is not defined
    ...
    useEffect(() => {
        ...
        if (user && user.role === 'admin') {
            ...
            // 🟢 กำหนดค่า State ว่า Admin โหลดข้อมูลเสร็จแล้ว
                ...
                <div className="page-view relative pb-20 animate-pop">
                    ...
                    <div className="space-y-4 pb-10">
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <select className="w-full text-xs p-2.5 border rounded-lg bg-slate-50 font-bold outline-none text-slate-700" value={dashJobType} onChange={e=>setDashJobType(e.target.value)}>
                                    <option value="All">ทุกประเภท</option><option value="New">New</option><option value="MOD">MOD</option>
                                </select>
                            </div>
                        </div>
                        ...
                        {(() => {
                            ...
                            const jobTypeCounts = allTasks.reduce((acc, t) => { acc[t.job_type || 'ไม่ระบุ'] = (acc[t.job_type || 'ไม่ระบุ'] || 0) + 1; return acc; }, {});
                            const docStats = allTasks.reduce((acc, t) => {
                                const isComplete = String(t.layout_doc)==='true' && String(t.wiring_doc)==='true' && String(t.precheck_doc)==='true';
                            ...
                            const inspectorCounts = allTasks.reduce((acc, t) => { acc[t.inspector_name || 'ไม่ระบุ'] = (acc[t.inspector_name || 'ไม่ระบุ'] || 0) + 1; return acc; }, {});
                            const sortedInspectors = (db.inspectors || []).map(ins => ({ name: ins.name, count: inspectorCounts[ins.name] || 0 })).sort((a,b) => b.count - a.count);
                                    ...
                                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                                        ...
                                        <div className="space-y-2">
                                            {sortedInspectors.map((ins, idx) => (
                                                <div key={ins.name} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full flex items-center justify-center font-black text-sm bg-indigo-100 text-indigo-600">{idx + 1}</div>
                ...
                <div className="page-view relative pb-20">
                    <div className="sticky top-0 bg-[#f1f5f9] z-10 pb-4 pt-2 border-b border-slate-200 mb-4">
                        <div className="flex justify-between items-center">
                            ...
                            {adminTab !== 'menu' && (
                                <button onClick={() => setAdminTab('menu')} className="text-xs font-bold text-slate-500 bg-white px-3 py-1.5 rounded-lg border shadow-sm flex items-center gap-1"><Icons.ChevronLeft /> กลับเมนู</button>
                            )}
                        </div>
                    </div>
                    ...
                    {adminTab === 'menu' && (
                        <div className="grid grid-cols-2 gap-4 animate-pop pb-10">
                            <button onClick={() => setAdminTab('users')} className="p-5 bg-white rounded-2xl shadow-sm border flex flex-col items-center gap-3"><div className="w-12 h-12 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center"><Icons.User /></div><span className="font-bold text-slate-700 text-sm">จัดการผู้ใช้งาน</span></button>
                            <button onClick={() => setAdminTab('inspectors')} className="p-5 bg-white rounded-2xl shadow-sm border flex flex-col items-center gap-3"><div className="w-12 h-12 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center"><Icons.FileCheck /></div><span className="font-bold text-slate-700 text-sm">ผู้ตรวจ & Certificate</span></button>
                            <button onClick={() => setAdminTab('special_management')} className="p-5 bg-white rounded-2xl shadow-sm border flex flex-col items-center gap-3 col-span-2"><div className="w-12 h-12 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center"><Icons.Clock /></div><span className="font-bold text-amber-800 text-sm">จัดการวันกิจกรรม/วันลา/วันหยุด</span></button>
                            <button onClick={() => setAdminTab('all_bookings')} className="p-5 bg-white rounded-2xl shadow-sm border flex flex-col items-center gap-3 col-span-2"><div className="w-12 h-12 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center"><Icons.List /></div><span className="font-bold text-slate-700 text-sm">งานทั้งหมดในระบบ</span></button>
                            <button onClick={() => setAdminTab('web_settings')} className="p-5 bg-white rounded-2xl shadow-sm border flex flex-col items-center gap-3 col-span-2"><div className="w-12 h-12 rounded-full bg-pink-50 text-pink-600 flex items-center justify-center"><Icons.Settings /></div><span className="font-bold text-slate-700 text-sm">ตั้งค่าสีเว็บไซต์ / ตาราง</span></button>
                        </div>
                    )}
                    ...
                    {adminTab === 'users' && (
                        <div className="space-y-3 mt-4 animate-pop pb-10">
                            <div className="flex justify-between items-center px-1 mb-3">
                                <h3 className="font-bold text-slate-800 text-sm">จัดการผู้ใช้งาน ({(adminDb.users || []).length})</h3>
                                <button onClick={() => setModal({ type: 'edit_user', isNew: true, data: { username: '', full_name: '', department: '', position: '', phone: '', role: 'user', status: 'approved' } })} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 shadow-sm"><Icons.Plus /> สร้างผู้ใช้</button>
                            </div>
                            {(adminDb.users || []).map((u, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border flex flex-col gap-2">
                                    <div className="flex justify-between items-center">
                                        <div><div className="font-bold text-slate-800 text-sm">{u.username} ({u.full_name || '-'})</div><div className="text-[10px] text-slate-500">สิทธิ์: {u.role} | สถานะ: {u.status}</div></div>
                                        <button onClick={() => setModal({ type: 'edit_user', isNew: false, data: u })} className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold border">แก้ไข</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {adminTab === 'inspectors' && (
                        <div className="space-y-4 pb-10">
                            <button onClick={() => setModal({ type: 'inspector_form' })} className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-md">เพิ่มผู้ตรวจใหม่</button>
                            {(db.inspectors || []).map((ins, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border flex justify-between items-center">
                                    ...
                                    <button onClick={() => setModal({ type: 'inspector_form', data: ins })} className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold border">แก้ไข</button>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {adminTab === 'special_management' && (
                        <div className="space-y-3">
                            <button onClick={() => setModal({ type: 'manage_leaves' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border font-bold text-amber-800 text-left">🌴 จัดการวันลา</button>
                            <button onClick={() => setModal({ type: 'manage_events' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border font-bold text-emerald-800 text-left">⭐ จัดการกิจกรรมบริษัท</button>
                            <button onClick={() => setModal({ type: 'manage_holidays' })} className="w-full p-4 bg-white rounded-2xl shadow-sm border font-bold text-red-800 text-left">❌ จัดการวันหยุดบริษัท</button>
                        </div>
                    )}
                        ...
                        <div className="space-y-3 pb-10">
                            {(adminDb.all_bookings || []).slice(0, adminBookingsLimit).map((h, i) => (
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border">
                                    <div className="font-bold text-slate-800 text-sm">{h.site_name} ({formatSafeDate(h.date)})</div>
                                    <div className="text-xs text-slate-500">ผู้ตรวจ: {h.inspector_name}</div>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {adminTab === 'web_settings' && (
                        <form onSubmit={async (e) => {
                            ...
                            <div><label className="text-xs font-bold block mb-1">ชื่อแอป (App Name)</label><input type="text" name="appName" defaultValue={db.settings?.appName || 'SAIS BOOKING'} className="w-full p-2.5 border rounded-lg text-sm" /></div>
                            <div><label className="text-xs font-bold block mb-1">สีหัว Header</label><input type="color" name="headerBg" defaultValue={db.settings?.headerBg || '#1e293b'} className="w-full h-10 border rounded cursor-pointer" /></div>
                            <button type="submit" className="w-full py-3 bg-slate-900 text-white font-bold rounded-xl">บันทึกการตั้งค่า</button>
                        </form>
                    )}
                </div>
            )}

            {/* 📍 Modals ควบคุมทั้งหมด */}
            {modal && (
                    ...
                    {modal?.type === 'edit_user' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if(modal.isNew) { payload.username = fd.get('username'); payload.password = fd.get('password'); }
                                if(window.dbFirestore) await window.dbFirestore.collection("users").doc(modal.isNew ? payload.username : modal.data.username).set(payload, { merge: true });
                                setSuccessModal('บันทึกสำเร็จ!'); setModal(null);
                            }} className="space-y-3">
                                {modal.isNew && <div><label className="text-xs font-bold">Username</label><input name="username" required className="w-full p-2 border rounded" /></div>}
