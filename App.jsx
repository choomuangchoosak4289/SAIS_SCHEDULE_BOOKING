...
// 📍 ดึงสีพื้นหลัง/อักษร รองรับการตั้งค่าสีอิสระของกิจกรรม
...
// 📍 CalendarGrid: แก้ไขไม่ให้สีวันหยุด/กิจกรรม เลอะทะลุไปยังแถบวันที่ด้านซ้าย
const CalendarGrid = React.memo(({ daysInView, db, isAdmin, user, setModal, setAlertMsg, handleDrop, handleDragOver, handleDragLeave, handleDragStart, handleDragEnd, setConfirmDialog, apiAction, setQuickAddType, filteredBookings, tableFontScale, columnZoom, specialFontScale, isExporting }) => {
    ...
    // 📍 ประมวลผลข้อมูลลงช่องตาราง (ยึดตามวันและชื่อผู้ตรวจ)
    ...
    return (
        <div id="calendar-export-area" className={`calendar-grid ${isExporting ? 'export-mode' : ''}`} style={{ 
            ...
            {daysInView.map((d, index) => {
                // 📍 แก้ไข: กำหนดให้ตัวหนังสือสีแดงสำหรับวันอาทิตย์ ไม่เปลี่ยนสีพื้นหลังให้เลอะทะลุ
                ...
                return (
                    <React.Fragment key={index}>
                        ...
                        {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                            ...
                            // เช็คว่าเซลล์นี้มีการลาหยุดหรือไม่ (เพื่อล็อคไม่ให้จอง)
                            ...
                            return (
                                <div key={idx} 
                                    ...
                                    onClick={() => {
                                        ...
                                        if (user.role === 'viewer') return; // 📍 สิทธิ์ Viewer คลิกลงตารางเปล่าไม่ได้
...
const App = () => {
    ...
    // 💡 availableInspectors มีการเรียกใช้ modal.data?.username จึงต้องประกาศ modal ก่อนหน้านี้
    ...
    // 📍 เพิ่ม State ควบคุมกิจกรรมบริษัทแบบหลายคน และเลือกสีอิสระ
    const [eventStartDate, setEventStartDate] = useState('');
    const [eventEndDate, setEventEndDate] = useState('');
    const [eventInspectors, setEventInspectors] = useState([]);
    const [showEventDropdown, setShowEventDropdown] = useState(false);
    const [eventColor, setEventColor] = useState('#22c55e'); // สีเขียวเริ่มต้น
    ...
    // 📍 ฟังก์ชัน API ส่งเข้า Google Sheets (ทำงานเบื้องหลัง ไม่บล็อกหน้าจอ)
    const apiAction = async (payload, customLoadMsg = null, disableAutoSync = false) => {
        ...
        try {
            ...
            const fetchPromise = fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify(payloadWithAuth) })
                .then(res => res.json())
                .then(result => {
                    // ปิดการแจ้งเตือน "ไม่พบ ID" 100% ตามที่ขอ
            ...
            // ถ้ามีโหลดข้อความให้รอจนเสร็จ ถ้าไม่มี(ทำงานเบื้องหลัง) ให้ผ่านไปเลย
            if (customLoadMsg) {
                await fetchPromise;
                setLoadingMsg(null);
            }
            ...
            if (!disableAutoSync && hasLoadedAdmin) {
                fetchAdminData(0, 50, 'all'); // Refresh admin data in background
            }
            return true;
        } catch (e) { 
    ...
    const handleBulkDelete = async (type, ids) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                // 📍 ลบจาก Firebase ทันทีแบบไม่ต้องรอ (Realtime)
    ...
    useEffect(() => {
        ...
        if (user && isFirebaseReady && window.dbFirestore) {
            unsubscribe = window.dbFirestore.collection("bookings").onSnapshot((snapshot) => {
                ...
                setDb(prev => ({ ...prev, bookings: firestoreBookings })); // ใช้อันนี้เป็นหลัก 100% ไม่พึ่ง Sheets แล้ว
                setLastSyncTime(getThaiTime());
                setInitialLoad(false);
            }, (err) => {
                console.error("Firebase Snapshot Error:", err);
            });
            fetchCoreData(); // ดึงแค่ Inspectors กับ Settings
        } else if (user) {
            fetchCoreData();
        }
    ...
    // 📍 โหลดแค่ข้อมูลรองจาก Sheets (เพราะ Bookings ใช้ Firebase แล้ว) ช่วยให้เร็วขึ้นมหาศาล
    const fetchCoreData = async () => {
        if (!SCRIPT_URL || !user) return;
        try {
            const res = await fetch(SCRIPT_URL, { 
                method: 'POST', 
                body: JSON.stringify({ action: 'sync_core', api_key: window?.SAIS_CONFIG?.API_KEY }) 
            });
        ...
        } catch (e) { console.error("Fetch Auxiliary Data Error"); }
    };
    ...
    // ค้นหา ID อัตโนมัติถ้าไม่มี ID (ป้องกัน Error ไม่พบไอดี)
    ...
    // 📍 ยกเลิกคิว (Background Sync แก้ค้าง 100%)
    const handleCancelBooking = (booking) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการลบ'); }
            }
        });
    };

    // 📍 ยกเลิกคิวแบบกรอกเหตุผล (Background Sync แก้ค้าง 100%)
        ...
        setPromptDialog({
            ...
            onSubmit: async (reason) => {
                ...
                try {
                    ...
                    apiAction({ action: 'delete_booking', id: docId || booking.id || '', user: user?.username || 'admin', reason: logDetail, job_type: booking.job_type, equipment_no: booking.equipment_no }, null, true);
                } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการยกเลิก'); }
            }
        });
    };

    // 📍 ตรวจสอบเอกสาร (Background Sync แก้ค้าง 100%)
    ...
    const handleFileUpload = async (e, docType, isMultiple = false) => {
        ...
        setLoadingMsg('กำลังบีบอัดรูปภาพและจัดเก็บ...'); // ฟังก์ชันนี้ต้องรอเพราะอัปโหลดไฟล์จริง
    ...
    // 📍 ทิ้งถังขยะแบบไม่ต้องรอ (Background Sync แก้ค้าง 100%)
    ...
    // 📍 ลากย้ายคิวแบบไม่ต้องรอ (Background Sync แก้ค้าง 100%)
        ...
        let confirmMsgNode = (
            <div className="text-left mt-2">
                ...
                <div className="mt-3 text-[10px] text-red-500 text-center font-bold">
                    *เมื่อกดยืนยัน ระบบจะย้ายคิวให้ทันที*
                </div>
            </div>
        );
    ...
    // 📍 บันทึก/แก้ไข คิวงานและข้อมูลพิเศษ (Background Sync แก้ค้าง 100%)
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            } catch(e) { console.error(e); setAlertMsg('ข้อผิดพลาดในการบันทึก'); }
            return;
        }
        ...
        } catch(e) { console.error(e); setAlertMsg('เกิดข้อผิดพลาดในการบันทึก'); }
    };
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            if (isForgotMode) {
                                ...
                                setSuccessModal('กำลังส่งข้อมูลสมัคร...');
                            ...
                            {isRegisterMode && (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {/* 📍 ช่องแผนกเป็น Text อิสระตามที่ขอครับ */}
        ...
        <div className="app-container">
            ...
            {modal && (
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                        ...
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.isNew) {
                                    ...
                                    setModal(null);
                                    setSuccessModal('กำลังส่งข้อมูลสมัคร...');
                                    const regOk = await apiAction(payload, null, true);
                                    if (regOk) {
                                        await apiAction({
                                    ...
                                    setModal(null);
                                    setSuccessModal('กำลังอัปเดตข้อมูล...');
                                    const ok = await apiAction(payload, null);
                                    if (ok) { setSuccessModal('อัปเดตข้อมูลผู้ใช้สำเร็จ'); fetchAdminData(0, 50, 'users'); }
                                }
                            }} className="space-y-3">
                                ...
                                {/* 📍 ช่องแผนก กลับมาเป็นช่อง Text พิมพ์เอาเองเหมือนเดิม */}
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div><label className="text-xs font-bold text-slate-700 block mb-1">แผนก</label><input type="text" name="department" defaultValue={modal.data.department} placeholder="NI, MOD, FQE" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none" /></div>
                                    <div><label className="text-xs font-bold text-slate-700 block mb-1">ตำแหน่ง</label><input type="text" name="position" defaultValue={modal.data.position} placeholder="PE, Tech" required className="w-full text-sm p-2.5 rounded-lg border border-slate-300 outline-none" /></div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-slate-100">
                                    <div className="bg-blue-50 p-2 rounded-lg border border-blue-100">
                                        ...
                                        <select name="role" defaultValue={modal.data.role || 'user'} onChange={(e) => {
                                            // 📍 ซ่อน/แสดงตัวเลือกผูกไอดี เฉพาะ Inspector เท่านั้นถึงจะเห็น
                                            const mapContainer = document.getElementById('mapping_name_container');
                                            if (mapContainer) mapContainer.style.display = e.target.value === 'inspector' ? 'block' : 'none';
                                        }} className="w-full text-sm p-2.5 rounded-lg border border-blue-200 outline-none font-bold bg-white text-blue-900">
                                            <option value="user">User ทั่วไป</option>
                                            <option value="inspector">Inspector (ผู้ตรวจ)</option>
                                            <option value="admin">Admin ผู้ดูแล</option>
                                            <option value="viewer">Viewer (สิทธิ์ผู้ชม)</option>
                                        </select>
                                    </div>
                                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                        ...
                                        <select name="status" defaultValue={modal.data.status || 'approved'} className="w-full text-sm p-2.5 rounded-lg border border-slate-200 outline-none font-bold bg-white text-slate-700">
                                            <option value="approved">✅ อนุมัติ</option><option value="pending">⏳ รอตรวจสอบ</option><option value="blocked">⛔ ระงับสิทธิ์</option>
                                        </select>
                                    </div>
                                </div>
                                
                                {/* 📍 แสดงเฉพาะตอนโหลดมาครั้งแรกเป็น Inspector หรือถูก onChange เปลี่ยนเป็น Inspector */}
                                <div id="mapping_name_container" style={{ display: modal.data.role === 'inspector' ? 'block' : 'none' }} className="bg-amber-50 p-3 rounded-xl border border-amber-200 mb-2 mt-2 shadow-sm">
                                    <label className="text-xs font-bold text-amber-800 block mb-1">🔗 ผูกชื่อในตารางปฏิทิน</label>
                                    ...
                                    <p className="text-[10px] text-amber-700 mt-1">ใช้ผูกเพื่อให้หน้า <b>"งานฉัน"</b> แสดงข้อมูลถูกต้องสำหรับช่างคนนี้</p>
                                </div>
                    ...
                    {modal?.type === 'inspector_form' && (
                        <div className="modal-card p-6 w-full max-w-md bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                setModal(null);
                                setSuccessModal('กำลังบันทึกข้อมูลผู้ตรวจ...');
                                const ok = await apiAction(payload, null);
                                if(ok) { setSuccessModal('บันทึกข้อมูลผู้ตรวจสำเร็จ'); }
                            }}>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button type="submit" className="w-2/3 py-3.5 bg-indigo-600 text-white font-bold rounded-xl shadow-md active:scale-95 transition-all flex items-center justify-center gap-2">
                                        <Icons.Check /> {modal.data ? 'บันทึกการแก้ไข' : 'ยืนยันเพิ่มผู้ตรวจ'}
                                    </button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.data?.id) {
                                    ...
                                    setSuccessModal('อัปเดตวันลาสำเร็จ'); setModal(null);
                                    ...
                                    setSuccessModal('แจ้งวันลาสำเร็จ'); setModal(null);
                    ...
                    {/* 📍 Modal จัดการกิจกรรม (อัปเกรด เลือกลายคน + เลือกสีอิสระ + ไม่โหลดค้าง) */}
                            ...
                            <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-slate-50 space-y-4">
                                <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-100">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {/* 📍 Color Picker สำหรับเลือกสีกิจกรรมอิสระแบบไม่กวนโค้ดระบบหลัก */}
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            // ถ้าเลือกทุกคน ให้เป้าหมายคือ SYSTEM_EVENT
