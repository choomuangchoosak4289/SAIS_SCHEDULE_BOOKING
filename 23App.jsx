...
const App = () => {
    ...
    // 📍 สร้าง ID จำลองเพื่อให้ Firestore และ Google Sheets มี ID อ้างอิงตรงกัน
    const generateId = () => {
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (window.dbFirestore) {
                        await window.dbFirestore.collection("bookings").doc(String(booking.id)).delete();
                    }
                    const ok = await apiAction({ action: 'delete_booking', id: booking.id, user: user?.username || 'admin', reason: logDetail, job_type: booking.job_type, equipment_no: booking.equipment_no }, null);
                    if(ok) { setSuccessModal('ลบสำเร็จ'); setModal(null); }
                } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการลบ: ' + e.message); setLoadingMsg(null); }
            }
        });
    };
    ...
    const handleCancelJob = (booking) => {
        ...
        setPromptDialog({
            ...
            onSubmit: async (reason) => {
                ...
                } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการยกเลิก'); setLoadingMsg(null); }
            }
        });
    };
    ...
    const handleVerifyDoc = async (booking, docField, isChecked) => {
        ...
        try {
            ...
            if (window.dbFirestore) {
                await window.dbFirestore.collection("bookings").doc(String(booking.id)).update({ [docField]: val });
            }
            const payload = { ...booking, action: 'update_booking', [docField]: val, reason: logDetail };
    ...
    // 📍 ฟังก์ชันอัปโหลดไฟล์แปลเป็น Base64 ฟรี 100% ไม่พึ่ง Storage
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    const ok = await apiAction({ action: 'delete_booking', id: task.id, user: user?.username || 'admin', reason: logDetail, job_type: task.job_type, equipment_no: task.equipment_no }, null);
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        } catch (e) { setAlertMsg('เกิดข้อผิดพลาด'); setLoadingMsg(null); }
    };
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            } catch(e) { setAlertMsg('ข้อผิดพลาดในการบันทึก'); setLoadingMsg(null); }
            return;
        }
        ...
        } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการบันทึก'); setLoadingMsg(null); }
    };
                                ...
                                if (modal.data?.id) {
                                    ...
                                    if (window.dbFirestore) {
                                        for(let date of dates) {
                                            ...
                                            await window.dbFirestore.collection("bookings").doc(newId).set({ id: newId, date: date, inspector_name: targetName, job_type: 'leave', site_name: lType, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                        }
                                    }
                                    const ok = await apiAction({ action: 'create_multiple_bookings', dates: dates, inspector_name: targetName, job_type: 'leave', site_name: lType, equipment_no: eqNo, user: user?.username, reason: 'แจ้งวันลาส่วนตัว' }, 'กำลังบันทึก...');
                                            ...
                                            for (let target of targets) {
                                                ...
                                                if (window.dbFirestore) {
                                                    for (let date of leaveDates) {
                                                        const newId = generateId();
                                                        await window.dbFirestore.collection("bookings").doc(newId).set({
                                                            id: newId, date: date, inspector_name: target, job_type: 'leave', site_name: finalType, equipment_no: eqNo, created_by: user?.username, status: 'active'
                                                        });
                                                    }
                                                }
                                                const payload = { action: 'create_multiple_bookings', dates: leaveDates, inspector_name: target, job_type: 'leave', site_name: finalType, equipment_no: eqNo, user: user?.username, reason: logDetail };
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-sm border">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            const eqNo = `EVENT_${Date.now()}`;
                                            
                                            if (window.dbFirestore) {
                                                for(let date of eventDates) {
                                                    const newId = generateId();
                                                    await window.dbFirestore.collection("bookings").doc(newId).set({ id: newId, date: date, inspector_name: 'SYSTEM_EVENT', job_type: 'company_event', site_name: eName, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                                }
                                            }
                                            const payload = { action: 'create_multiple_bookings', dates: eventDates, inspector_name: 'SYSTEM_EVENT', job_type: 'company_event', site_name: eName, equipment_no: eqNo, user: user?.username, reason: 'สร้างกิจกรรม' };
                                            await apiAction(payload, null, true);
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            if (window.dbFirestore) {
                                                for(let date of holidayDates) {
                                                    const newId = generateId();
                                                    await window.dbFirestore.collection("bookings").doc(newId).set({ id: newId, date: date, inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: hName, equipment_no: eqNo, created_by: user?.username, status: 'active' });
                                                }
                                            }
                                            const ok = await apiAction({ action: 'create_multiple_bookings', dates: holidayDates, inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: hName, equipment_no: eqNo, user: user?.username, reason: 'เพิ่มวันหยุด' }, 'กำลังสร้างวันหยุด...');
