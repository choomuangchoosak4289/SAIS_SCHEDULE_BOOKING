...
// 📍 ฟังก์ชันบังคับล็อกโซนเวลาประเทศไทย (GMT+7) เสมอ[cite: 1]
...
// 📍 CalendarGrid ป้องกันตารางพังด้วย Inline Width 100%
          ...
          {/* 3. หน้าค้นหา */}
            ...
            {modal && (
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                    ...
                    {/* 📍 MODAL DETAIL (รายละเอียดงาน) อัปเดตข้อความ "ตรวจสอบแล้ว/รอตรวจสอบ" */}
                    {modal?.type === 'detail' && (
            ...
            {/* 📍 หน้าต่างแจ้งเตือนและประวัติ (แยก Tab แก้ปัญหา Error โค้ดสมบูรณ์) */}
            {showActivityModal && (
                <div className="backdrop z-[200]">
                    <div className="modal-card p-6 h-[85vh] flex flex-col relative">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-3 flex items-center gap-2 flex-shrink-0"><Icons.Bell /> การแจ้งเตือน & ประวัติ</h3>
                        
                        {/* แถบสลับมุมมองระหว่าง แจ้งเตือน กับ ประวัติระบบ */}
                        <div className="flex gap-2 mb-4 bg-slate-100 p-1 rounded-xl flex-shrink-0">
                            ...
                            <button onClick={() => { setActivityTab('log'); if(!hasLoadedAdmin) fetchAdminData(0, 50, 'logs'); }} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activityTab === 'log' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500'}`}>
                                🗄️ ประวัติระบบ (Logs)
                            </button>
                        </div>
                            ...
                            {activityTab === 'notif' ? (
                                (() => {
                                    ...
                                    return userNotifs.map((notif, i) => {
                                        const readers = String(notif.isRead || '').split(',');
                                        const isUnread = !readers.includes(user?.username);

                                        return (
                                            <div key={`notif_${notif.id || i}`} onClick={() => { 
                                                // เมื่อคลิกที่รายการแจ้งเตือน ให้ส่งค่าไปบันทึกว่าอ่านแล้วเฉพาะ User นี้
                                                if(isUnread) apiAction({ action: 'read_notification', id: notif.id, user: user?.username }, null, true); 
                                            }}
                                                 className={`p-4 rounded-xl border shadow-sm cursor-pointer transition-all ${isUnread ? 'bg-blue-50/80 border-blue-300 ring-1 ring-blue-300' : 'bg-white border-slate-200 opacity-75'}`}>
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="font-bold text-sm text-slate-800 flex items-center gap-2">
                                                        {isUnread && <span className="w-2.5 h-2.5 rounded-full bg-blue-500 animate-pulse"></span>}
                                                        {notif.title || 'แจ้งเตือนระบบ'}
                                                    </div>
                                                    <div className="text-[9px] text-slate-400 bg-slate-50 px-2 py-1 rounded-md border">
                                                        {notif.timestamp ? new Date(notif.timestamp).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                                                    </div>
                                                </div>
                                                <div className="text-xs text-slate-700 leading-relaxed whitespace-pre-line">{notif.message}</div>
                                            </div>
                                        );
                                    });
                                })()
                                ...
                                (() => {
                                    if (!hasLoadedAdmin) return <div className="text-center text-slate-400 p-8"><Icons.Loader /> กำลังโหลดประวัติระบบ...</div>;
                                    
                                    const logsList = (adminDb.logs || []).sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));
                                    if (logsList.length === 0) return <div className="text-center text-slate-400 text-sm py-12">ไม่พบประวัติระบบ</div>;

                                    return logsList.slice(0, logsLimit).map((log, i) => (
                                        <div key={`log_${i}`} className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex items-center gap-2">
