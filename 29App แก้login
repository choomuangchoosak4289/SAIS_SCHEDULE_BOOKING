...
const App = () => {
    ...
    // 📍 ประกาศ state 'modal' ไว้บนสุด ป้องกัน Error "modal is not defined" 100%
    ...
    // 📍 ฟังก์ชันย้ายฐานข้อมูลจาก Sheets ไป Firestore (ฉบับสมบูรณ์)
    const handleMigrateToFirestore = async () => {
        // ส่วนที่ 1: ตรวจสอบความพร้อมก่อนย้ายข้อมูล
        if (!window.confirm("ยืนยันการย้ายข้อมูลทั้งหมดจาก Sheets ลง Firestore ใช่หรือไม่?\n(กรุณารอจนกว่าระบบจะแจ้งเตือนว่าสำเร็จ)")) return;
        ...
        try {
            let stats = { bookings: 0, users: 0, inspectors: 0, notifications: 0, settings: 0 };
            const batch = window.dbFirestore.batch(); // ใช้ Batch ช่วยเขียนข้อมูลรวดเดียว

            // ส่วนที่ 2: แพ็คข้อมูล Bookings
            setLoadingMsg("กำลังย้ายข้อมูล Bookings (1/5)...");
            ...
            // ส่วนที่ 3: แพ็คข้อมูล Users (ใช้ username เป็น Document ID เพื่อให้ล็อกอินได้)
            setLoadingMsg("กำลังย้ายข้อมูล Users (2/5)...");
            ...
            // ส่วนที่ 4: แพ็คข้อมูล Inspectors
            setLoadingMsg("กำลังย้ายข้อมูล Inspectors (3/5)...");
            ...
            // ส่วนที่ 5: แพ็คข้อมูล Notifications & Settings
            setLoadingMsg("กำลังย้ายข้อมูล Notifications & Settings (4/5)...");
            const notifications = db.notifications || [];
            for (const n of notifications) {
                if (!n.id) continue;
                const docRef = window.dbFirestore.collection("notifications").doc(String(n.id));
                batch.set(docRef, n);
                stats.notifications++;
            }
            ...
            if (Object.keys(settings).length > 0) {
                const docRef = window.dbFirestore.collection("settings").doc("web_settings");
                batch.set(docRef, settings);
                stats.settings = 1;
            }

            // ส่วนที่ 6: สั่งอัปโหลดข้อมูลทั้งหมดขึ้น Firebase พร้อมกัน
            setLoadingMsg("กำลังบันทึกลงฐานข้อมูล Firebase... (อย่าเพิ่งปิดหน้าจอ) (5/5)");
            await batch.commit();

            setLoadingMsg(null);
            setSuccessModal(`ย้ายข้อมูลลง Firestore สำเร็จ 100%!\n- คิวงาน: ${stats.bookings} รายการ\n- บัญชีผู้ใช้: ${stats.users} บัญชี\n- รายชื่อผู้ตรวจ: ${stats.inspectors} คน\n- แจ้งเตือน: ${stats.notifications} รายการ`);
            
        } catch (error) {
            setLoadingMsg(null);
            console.error("Migration Error:", error);
            setAlertMsg("เกิดข้อผิดพลาดในการย้ายข้อมูล: " + error.message);
        }
    };
    ...
    const handleTouchEnd = async () => {
        if (pullY > 50 && !isRefreshing) { 
            ...
            setPullY(50); 
            // ไม่ต้องทำ fetchCoreData แล้วเพราะ OnSnapshot ทำงานแทน ให้โชว์แอนิเมชันหลอกนิดหน่อย
    ...
    // 📍 ฟังก์ชันเรียก Logs ทั่วไปที่ไม่ได้กระทบ Core CRUD
    ...
    // 📍 ใช้งาน Firebase ลบข้อมูลแบบกลุ่ม
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    setLoadingMsg(null);
                    setAlertMsg('เกิดข้อผิดพลาดในการลบข้อมูล');
                }
            }
        });
    };
    ...
    // 📍 ดึงข้อมูล Realtime ด้วย Firestore (Compat) 100% 
    useEffect(() => {
        let unsubs = [];
        if (user && isFirebaseReady && window.dbFirestore) {
            ...
            }, (err) => console.error("Firebase Bookings Error:", err)));

            unsubs.push(window.dbFirestore.collection("inspectors").onSnapshot((snapshot) => {
                const firestoreInspectors = [];
                snapshot.forEach(doc => firestoreInspectors.push(doc.data()));
                setDb(prev => ({ ...prev, inspectors: firestoreInspectors })); 
            }));

            unsubs.push(window.dbFirestore.collection("notifications").orderBy("timestamp", "desc").limit(50).onSnapshot((snapshot) => {
                const notifs = [];
                snapshot.forEach(doc => notifs.push(doc.data()));
                setDb(prev => ({ ...prev, notifications: notifs })); 
            }));
            ...
            // ดึงข้อมูลแอดมิน (Users) เข้ามาเก็บไว้ใช้งาน
    ...
    // เลิกใช้ fetchCoreData
    const fetchCoreData = async () => { return true; };

    // 📍 ดึงข้อมูล Admin Data (Users) จาก Firestore แทน
    const fetchAdminData = async (offset = 0, limit = 50, fetchType = 'all') => {
        if (!user || !window.dbFirestore) return;
        try {
            if (fetchType === 'all' || fetchType === 'users') {
                const usersSnap = await window.dbFirestore.collection("users").get();
                const usersData = usersSnap.docs.map(doc => doc.data());
                setAdminDb(prev => ({ ...prev, users: usersData }));
            }
            if (fetchType === 'all' || fetchType === 'bookings') {
                const bookingsSnap = await window.dbFirestore.collection("bookings").get();
                const bookingsData = bookingsSnap.docs.map(doc => doc.data());
                setAdminDb(prev => ({ ...prev, all_bookings: bookingsData }));
            }
    ...
    // 📍 ระบบค้นหา ID ฉลาด (ป้องกัน Error หา ID ไม่เจอใน Firebase)
    ...
    // 📍 ลบข้อมูลคิวงาน (ลบตรงจาก Firebase 100%)
    ...
    // 📍 ยกเลิกคิวงานพร้อมระบุเหตุผล (Firebase 100%)
    ...
    // 📍 ตรวจสอบเอกสาร (อัปเดต Firebase 100%)
    const handleVerifyDoc = async (booking, docField, isChecked) => {
        ...
        try {
            ...
            setLoadingMsg(null);
            setAlertMsg('อัปเดตสถานะไม่สำเร็จ'); 
        }
    };
    ...
    // 📍 ทิ้งถังขยะแบบไม่ต้องรอ (ลบตรงจาก Firebase 100%)
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    setAlertMsg('เกิดข้อผิดพลาดในการลบ: ' + e.message); 
                    setLoadingMsg(null); 
                }
            }
        });
    ...
    // 📍 ลากย้ายคิวแบบไม่ต้องรอ (อัปเดต Firebase 100%)
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (window.dbFirestore) {
                        await window.dbFirestore.collection("bookings").doc(String(task.id)).update({ date: targetDate, inspector_name: finalInspector });
                    }
                    setLoadingMsg(null);
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        try {
            ...
            if (window.dbFirestore) {
                await window.dbFirestore.collection("bookings").doc(String(modal.data.id)).update({ site_name: newTitle, inspector_name: newInspector, date: newDate });
            }
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (quickAddType !== 'job') {
            ...
            try {
                ...
                if (window.dbFirestore) {
                    await window.dbFirestore.collection("bookings").doc(newId).set({
                ...
                setAlertMsg('ข้อผิดพลาดในการบันทึก: ' + e.message); 
                setLoadingMsg(null); 
            }
            return;
        }
        ...
        try {
            ...
            if (window.dbFirestore) {
                await window.dbFirestore.collection("bookings").doc(String(bookingId)).set({ ...payload, status: 'active' });
            }
    ...
    // 📍 หน้าจอ Login (ระบบเข้าสู่ระบบ กู้รหัส และสมัครสมาชิก โฉมใหม่ Firebase 100%)
    if (!user) {
        return (
                ...
                <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 relative overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    {showLoginHelp ? (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            // 📍 โหมดล็อกอินเข้าสู่ระบบ (Firebase)
                            else {
                                setLoadingMsg('กำลังตรวจสอบข้อมูล...');
                                try {
                                    if (!window.dbFirestore) throw new Error("ระบบฐานข้อมูลยังไม่พร้อมทำงาน กรุณารอสักครู่");
        ...
        <div className="app-container">
                    ...
                    {showRoleHelp && (
                        <div className="bg-slate-800 text-white p-4 rounded-xl mb-4 shadow-md text-xs leading-relaxed animate-pop relative">
                            ...
                            <h3 className="font-bold text-sm mb-2 text-blue-300">
                                {user?.role === 'inspector' ? '🛠️ คู่มือสำหรับ Inspector' : '📌 คู่มือสำหรับผู้จอง'}
                            </h3>
                            {user?.role === 'inspector' ? (
                                ...
                                <button onClick={() => {
                                    setConfirmDialog({
                                        ...
                                        onConfirm: async () => {
                                            ...
                                            if (ok) { setSuccessModal('รีเซ็ตการตั้งค่าเรียบร้อยแล้ว'); fetchCoreData(); }
                                        }
                                    });
                                ...
                                <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-2 hover:border-blue-300 transition-all">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        ...
                                        <button onClick={() => {
                                            ...
                                            if (window.dbFirestore) {
                                                window.dbFirestore.collection("users").doc(u.username).update({ status: newStatus });
                                            }
                                        ...
                                        <button onClick={() => {
                                            setConfirmDialog({
                                                ...
                                                onConfirm: async () => {
                                                    if (window.dbFirestore) {
                                                        await window.dbFirestore.collection("users").doc(u.username).delete();
                                                    }
            ...
            {modal && (
                <div className="backdrop z-[100] p-4 flex items-center justify-center">
                        ...
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.data?.id) {
                                    ...
                                    if (window.dbFirestore) {
                                        await window.dbFirestore.collection("bookings").doc(String(modal.data.id)).update({ date: sDate, site_name: lType });
                                    }
                                    apiAction({ action: 'update_booking', id: modal.data.id, date: sDate, site_name: lType, user: user?.username, reason: 'แก้ไขวันลาส่วนตัว' }, null, true);
                                    setLoadingMsg(null);
                    ...
                    {/* Modal จัดการวันลา (อัปเกรด Firebase Background Sync) */}
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="w-full p-2.5 text-sm border rounded-lg bg-slate-50 font-bold cursor-pointer flex justify-between items-center" onClick={() => setShowEventDropdown(!showEventDropdown)}>
                                                <span className={`truncate ${eventInspectors.length === 0 ? 'text-slate-400' : 'text-emerald-700'}`}>
                                                    {eventInspectors.length === 0 ? '-- เลือกผู้เข้าร่วม --' : eventInspectors.includes('ALL') ? '✅ ทุกคน (แสดงแถบยาวตลอดวัน)' : `👤 ผู้ตรวจ (${eventInspectors.length} คน)`}
                                                </span>
                                                <span className="text-slate-400 text-xs">▼</span>
                                            </div>
                                            {showEventDropdown && (
