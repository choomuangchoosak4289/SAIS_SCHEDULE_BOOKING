...
const App = () => {
    ...
    const handleBulkDelete = async (type, ids) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                // ลบจาก Firebase ทันทีแบบไม่ต้องรอ
    ...
    // 📍 ดึงข้อมูล Realtime ด้วย Firestore (Compat)
    useEffect(() => {
        let unsubscribe;
    ...
    const fetchCoreData = async (needPast = false, currentCache = null) => {
        ...
        try {
            ...
            if (result.status === 'ok') {
                ...
                // Update เฉพาะ settings และ inspectors (เพราะ bookings ใช้ Firestore เป็นหลัก)
                setDb(prev => ({...prev, settings: finalDb.settings, inspectors: finalDb.inspectors, notifications: finalDb.notifications}));
    ...
    const handleCancelBooking = (booking) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    // ลบจาก Firebase ทันที
                    if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(booking.id)).delete();
                    setSuccessModal('ลบสำเร็จ');
                    
                    // สั่ง Backup ไป Google Sheets (เบื้องหลัง)
                    apiAction({ action: 'delete_booking', id: booking.id, user: user?.username || 'admin', reason: logDetail, job_type: booking.job_type, equipment_no: booking.equipment_no }, null, true);
                } catch(e) { setAlertMsg('เกิดข้อผิดพลาดในการลบ: ' + e.message); }
            }
        });
    };
    ...
    const handleVerifyDoc = async (booking, docField, isChecked) => {
        ...
        try {
            if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(booking.id)).update({ [docField]: val });
            setSuccessModal(`อัปเดตเอกสาร ${docName} สำเร็จ`);
            
            apiAction({ ...booking, action: 'update_booking', [docField]: val, reason: logDetail }, null, true);
    ...
    const handleTrashDrop = async (e) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(task.id)).delete();
                    setSuccessModal('ลบรายการลงถังขยะสำเร็จ');
                    
                    apiAction({ action: 'delete_booking', id: task.id, user: user?.username || 'admin', reason: logDetail, job_type: task.job_type, equipment_no: task.equipment_no }, null, true);
    ...
    const handleDrop = async (e, targetDate, targetInspector) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                try {
                    // อัปเดตลง Firebase ทันที ไม่ต้องรอ
                    if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(task.id)).update({ date: targetDate, inspector_name: finalInspector });
    ...
    const handleEditSpecialSubmit = async (e) => {
        ...
        try {
            ...
            if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(modal.data.id)).update({ site_name: newTitle, inspector_name: newInspector, date: newDate });
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 MODALS ควบคุมการทำงานทั้งหมด (อิงจาก 21App 100% ไม่มีการตัดทิ้ง) */}
            {modal && (
                                            ...
                                            {showEventDropdown && (
                                                <div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-48 overflow-y-auto custom-scrollbar">
                                                    <label className="flex items-center gap-3 p-3 hover:bg-emerald-50 cursor-pointer border-b border-slate-100">
                                                        ...
                                                        <span className="font-black text-emerald-700 text-sm">✅ ทุกคน (กิจกรรมรวม)</span>
                                                    </label>
                                                    {(db.inspectors || []).map(i => (
