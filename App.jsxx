...
const App = () => {
    ...
    // 📍 1. โหลด Firebase Compat ผ่าน Script Tag (แก้ปัญหาจอขาวบน Apps Script 100%)
    useEffect(() => {
    ...
    // 📍 ระบบ API Action แบบลื่นไหล ไม่กวนหน้าจอตอนทำงาน
    ...
    const handleBulkDelete = async (type, ids) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                // 📍 แบ็คอัปคำสั่งลบไปที่ Google Sheets (เบื้องหลัง)
    ...
    // 📍 3. ดึงข้อมูล Realtime ด้วย Firestore + Hybrid Fallback 100%
    useEffect(() => {
        let unsubscribe;
        if (user && isFirebaseReady && window.dbFirestore) {
            unsubscribe = window.dbFirestore.collection("bookings").onSnapshot((snapshot) => {
                ...
                // แต่ถ้า Firebase ว่าง (เพิ่งเข้าใช้งาน ยังไม่ย้ายข้อมูล) ให้ใช้ของ Google Sheets ไปก่อน
    ...
    const fetchCoreData = async (needPast = false, currentCache = null) => {
        ...
        try {
            ...
            if (result.status === 'ok') {
                ...
                // 📍 อัปเดตข้อมูลรอง แต่ถ้า bookings มีใน Firebase แล้ว จะไม่ให้ Sheets ทับ
                setDb(prev => ({ 
                    ...prev, 
    ...
    const handleTabChange = (view) => {
        setCurrentView(view);
        // 📍 โหลดข้อมูลสถิติเสมอเมื่อเปลี่ยนแท็บ
    ...
    // 📍 หน้าจอ Login (อัปเกรด Hybrid Firebase + Sheets เพื่อแก้ปัญหา Login ครั้งแรกไม่ได้)
    if (!user) {
        return (
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            // 📍 สมัครสมาชิกใหม่แบบ Hybrid
                                ...
                                try {
                                    ...
                                    // เขียนลง Firebase
                                    if (window.dbFirestore) await window.dbFirestore.collection("users").doc(usernameInput).set(payload);
                            ...
                            // 📍 เข้าสู่ระบบแบบ Hybrid (แก้ปัญหา Firebase ว่างเปล่า)
                            else {
                                ...
                                try {
                                    ...
                                    // 2. ถ้าใน Firebase ยังไม่มีข้อมูล (เพิ่งติดตั้งใหม่) ให้ค้นหาผ่าน Google Sheets แทน
                                    if (!userData && SCRIPT_URL) {
                                        ...
                                        if (result.status === 'ok') {
                                            ...
                                            // นำข้อมูลผู้ใช้ไปเขียนลง Firebase เพื่อให้ล็อกอินครั้งต่อไปเร็วขึ้น
                                            if (window.dbFirestore) {
