...
const App = () => {
    ...
    // 📍 3. ดึงข้อมูล Realtime ด้วย Firestore (Compat)
    useEffect(() => {
        let unsubscribe;
    ...
    // 📍 ฟังก์ชันอัปโหลดไฟล์ (บีบอัดเป็น Base64 เก็บลง Firestore ฟรี 100% ไม่ต้องพึ่ง Storage)
                    ...
                    {modal?.type === 'inspector_leave_form' && (
                        <div className="modal-card p-6 w-full max-w-sm bg-white rounded-3xl shadow-2xl animate-pop relative">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (modal.data?.id) {
                                    if (window.dbFirestore) await window.dbFirestore.collection("bookings").doc(String(modal.data.id)).update({ date: sDate, site_name: lType });
